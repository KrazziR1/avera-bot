...
async def claim_bounty(ctx, bounty_id: int = None):
    ...
    bounty["pending_claim"] = {
        "user": ctx.author,
        "attachment": ctx.message.attachments[0].url
    }

    # Notify admins in the bounty channel
    bounty_channel = bot.get_channel(BOUNTY_CHANNEL_ID)

    embed = discord.Embed(
        title=f"⏳ Bounty #{bounty_id} Claim — Pending Approval",
        description=(
            f"{ctx.author.mention} is claiming the bounty on **{bounty['target']}**!\n\n"
            f"**Reward:** {bounty['points']} points\n\n"
            f"*Admins: Use `!approveclaim {bounty_id}` to approve or `!denyclaim {bounty_id}` to deny.*"
        ),
        color=discord.Color.orange()
    )
    ...
    await ctx.send(
        ...
        f"An admin will review your proof shortly. If approved, you'll receive **{bounty['points']} points**! 🎯"
    )
...
async def approve_claim(ctx, bounty_id: int = None):
    ...
    if "pending_claim" not in bounty or not bounty["pending_claim"]:
        await ctx.send(f"❌ No pending claim found for Bounty #{bounty_id}.")
        return

    claimer = bounty["pending_claim"]["user"]
    pts = bounty["points"]
    ...
    embed = discord.Embed(
        ...
        description=(
            f"🎉 {claimer.mention} has successfully claimed the bounty on **{bounty['target']}**!\n\n"
    ...
    await ctx.send(f"✅ Approved! **{claimer.display_name}** has been awarded **{pts} points** for Bounty #{bounty_id}.")
...
async def deny_claim(ctx, bounty_id: int = None, *, reason: str = "No reason provided."):
    ...
    if bounty_id not in bounties or "pending_claim" not in bounties[bounty_id]:
        await ctx.send(f"❌ No pending claim found for Bounty #{bounty_id}.")
        return
    ...
    await ctx.send(f"❌ Claim for Bounty #{bounty_id} denied.")
    try:
        await claimer.send(
            f"❌ Your claim for Bounty #{bounty_id} (**{bounty['target']}**) was denied.\n"
            f"**Reason:** {reason}\n"
            f"You may try again with better proof!"
        )
