...
async def help_command(ctx):
    ...
    embed.add_field(name="📅 Events", value=(
        ...
        f"`{PREFIX}startattendance <#>` — Open attendance for an event *(Admin only)*\n"
...
@commands.has_permissions(manage_guild=True)
async def start_attendance(ctx, event_number: int = None):
    """Start attendance tracking for an event. Admin only.
    Usage: !startattendance <event number>"""
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"*Attendance will be closed by an admin when the event ends.*"
        ),
        color=discord.Color.green()
    )
    ...
    if ctx.channel.id != EVENTS_CHANNEL_ID:
        await ctx.send(f"✅ Attendance tracking started for **{event['name']}**! Members can now type `!attend`.")
...
async def welcome_panel(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "🔗 **Discord Invite:** [discord.gg/Avera](https://discord.gg/Avera)"
        ),
        color=discord.Color.gold()
    )
    ...
    embed.add_field(
        ...
        value=(
            "🎯 **Custom Bounty & Point System** — Complete bounties, earn points, spend them in the clan shop\n"
            "📚 **Skill of the Week** — Weekly community skill grind\n"
            "💀 **Boss of the Week** — Weekly featured boss for the clan to slay\n"
