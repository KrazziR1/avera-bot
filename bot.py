...
async def fetch_temple_stats(rsn):
    ...
    try:
        url = f"https://templeosrs.com/api/player_info.php?player={rsn.replace(' ', '%20')}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status != 200:
                    return None, None
        ...
        # Collection log slots — TempleOSRS field
        collection_log = None
        for key in ["Collection_log_count", "Collectionlog", "collection_log", "Collection_log"]:
            val = info.get(key)
            if val is not None:
                try:
        ...
        # Achievement diaries — try multiple known field name formats
        def get_diary(tier):
            candidates = [
...
async def temple_debug(ctx, *, rsn: str = None):
    ...
    try:
        ...
        relevant = {k: v for k, v in info.items() if any(x in k.lower() for x in ["diary", "collection", "log"])}

        if relevant:
            lines = "\n".join(f"`{k}`: {v}" for k, v in list(relevant.items())[:20])
            await msg.edit(content=f"📊 Temple OSRS fields for **{rsn}**:\n{lines}")
        else:
            all_keys = ", ".join(list(info.keys())[:30])
            await msg.edit(content=f"⚠️ No diary/collection fields found for **{rsn}**.\nAll keys: {all_keys}")
    except Exception as e:
