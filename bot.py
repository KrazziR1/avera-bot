...
def calculate_glory(user_id, total_level=None, total_xp=None):
    ...
    # Total level: 1 pt per 10 levels
    if total_level:
        glory += total_level / 10

    # Total XP: 1 pt per 10M XP
    if total_xp:
        glory += total_xp / 10_000_000
...
async def glory_command(ctx, member: discord.Member = None):
    ...
    level_glory = round((total_level or 0) / 10, 1)
    xp_glory = round((total_xp or 0) / 10_000_000, 1)
    ...
    embed.add_field(
        ...
        value=(
            ...
            f"🧮 Total XP ({(total_xp or 0)/1_000_000:.0f}M): **+{xp_glory:.1f}**\n"
...
async def rank_progress(ctx, member: discord.Member = None):
    ...
    # Level milestone — next meaningful breakpoint above current level
    level_breakpoints = [750, 1000, 1250, 1500, 1750, 2000, 2200, 2376]
    current_level = total_level or 0
    next_milestone = next((bp for bp in level_breakpoints if bp > current_level), None)
    if next_milestone and next_milestone <= 2376:
        milestone_glory = round((next_milestone - current_level) / 10, 1)
        suggestions.append(f"📊 Reach **{next_milestone} total level** (+{milestone_glory:.0f} Glory toward your goal)")

    # XP milestone — next 50M or 100M block
    current_xp_m = (total_xp or 0) / 1_000_000
    if current_xp_m < 100:
        next_xp = ((int(current_xp_m) // 50) + 1) * 50
    else:
        next_xp = ((int(current_xp_m) // 100) + 1) * 100
    xp_milestone_glory = round((next_xp - current_xp_m) / 10, 1)
    suggestions.append(f"🧮 Reach **{next_xp}M total XP** (+{xp_milestone_glory:.0f} Glory toward your goal)")
...
async def faq_panel(ctx):
    ...
    embed3.add_field(
        ...
        value=(
            ...
            "📊 **Your in-game stats** — total level and total XP\n"
