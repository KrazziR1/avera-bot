...
async def fetch_gear_from_claude(boss_name: str, style: str, wikitext: str) -> dict | None:
    """Use Claude AI to extract and format gear info from wikitext into structured data."""
    prompt = f"""You are an OSRS expert. Extract the recommended gear setup for killing {boss_name} using {style} from the following OSRS Wiki wikitext.

Return ONLY a valid JSON object with exactly this structure — no markdown, no explanation, just the JSON:
{{
  "style": "{style}",
  "boss": "{boss_name}",
  "overview": "1-2 sentence summary of this style for this boss",
  "gear": {{
    "bis": {{"Head": "item name", "Neck": "item name", "Cape": "item name", "Body": "item name", "Legs": "item name", "Weapon": "item name", "Offhand": "item name or N/A", "Hands": "item name", "Feet": "item name", "Ring": "item name", "Ammo": "item name or N/A"}},
    "mid": {{"Head": "item name", "Neck": "item name", "Cape": "item name", "Body": "item name", "Legs": "item name", "Weapon": "item name", "Offhand": "item name or N/A", "Hands": "item name", "Feet": "item name", "Ring": "item name", "Ammo": "item name or N/A"}},
    "budget": {{"Head": "item name", "Neck": "item name", "Cape": "item name", "Body": "item name", "Legs": "item name", "Weapon": "item name", "Offhand": "item name or N/A", "Hands": "item name", "Feet": "item name", "Ring": "item name", "Ammo": "item name or N/A"}}
  }},
  "inventory": ["item1", "item2", "item3", "...up to 8 key items"],
  "prayers": ["prayer1", "prayer2"],
  "settings": "brief note on game settings e.g. autocast spell, attack style etc.",
  "notes": "any important tips or phase-specific notes, max 2 sentences"
}}

If a tier (mid/budget) is not meaningfully different from BiS for this boss, use the same items.
If a slot is not applicable, use "N/A".
...
{wikitext[:8000]}"""
    ...
    try:
        ...
        print(f"Claude gear parse error: {e}")
        return None


def build_gear_embed(gear_data: dict, style: str, boss_name: str) -> discord.Embed:
    """Build a rich Discord embed from parsed gear data."""
    emoji = STYLE_EMOJI.get(style, "⚔️")
    colour_map = {"Magic": 0x3498db, "Ranged": 0x2ecc71, "Melee": 0xe74c3c}
    base_style = next((k for k in colour_map if k in style), "Melee")
    colour = colour_map[base_style]

    embed = discord.Embed(
        title=f"{emoji}  {boss_name} — {style} Setup",
        description=gear_data.get("overview", ""),
        color=colour
    )
    embed.set_thumbnail(url=f"https://oldschool.runescape.wiki/images/{boss_name.replace(' ', '_')}.png")

    SLOTS = ["Head", "Neck", "Cape", "Body", "Legs", "Weapon", "Offhand", "Hands", "Feet", "Ring", "Ammo"]
    gear = gear_data.get("gear", {})

    def fmt_slot(tier_data):
        lines = []
        for slot in SLOTS:
            item = tier_data.get(slot, "N/A")
    ...
    bis    = gear.get("bis", {})
    mid    = gear.get("mid", {})
    budget = gear.get("budget", {})

    embed.add_field(name="🥇 BiS Setup",    value=fmt_slot(bis),    inline=True)
    embed.add_field(name="🥈 Mid Setup",    value=fmt_slot(mid),    inline=True)
    embed.add_field(name="🥉 Budget Setup", value=fmt_slot(budget), inline=True)

    inventory = gear_data.get("inventory", [])
    if inventory:
        embed.add_field(name="🎒 Inventory", value="\n".join(f"• {i}" for i in inventory), inline=False)

    prayers = gear_data.get("prayers", [])
    if prayers:
        embed.add_field(name="🙏 Prayers", value=", ".join(prayers), inline=True)

    settings = gear_data.get("settings", "")
    if settings:
        embed.add_field(name="⚙️ Settings", value=settings, inline=True)

    notes = gear_data.get("notes", "")
    if notes:
        embed.add_field(name="📝 Notes", value=notes, inline=False)

    embed.set_footer(text="Data sourced from the OSRS Wiki • Avera Clan")
    return embed
    ...
    def _make_callback(self, style: str):
        async def callback(interaction: discord.Interaction):
            ...
            if cached and (now_ts - cached.get("fetched_at", 0)) < GEAR_CACHE_TTL:
                gear_data = cached["data"]
                embed = build_gear_embed(gear_data, style, self.wiki_name)
                await interaction.response.send_message(embed=embed)
                self._loading = False
                return
            ...
            gear_data = await fetch_gear_from_claude(self.wiki_name, style, self.wikitext)
            if not gear_data:
                await interaction.followup.send("⚠️ Couldn't parse gear data from the wiki right now. Try again shortly!")
                self._loading = False
                return

            # Cache it
            gear_cache[cache_key] = {"data": gear_data, "fetched_at": now_ts}
