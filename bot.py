...
async def claim_bounty(ctx, bounty_id: int = None):
    ...
    if bounty_id in claim_channels:
        await ctx.send(f"❌ A claim for Bounty #{bounty_id} is already under review!")
        return
    ...
    # Confirm to the claimant in the original channel
    await ctx.send(
...
async def approve_claim(ctx, bounty_id: int = None):
    ...
    pts = bounty["points"]
    review_channel_id = bounty["pending_claim"].get("review_channel_id")
    ...
    # Announce in the review channel, then delete it after a short delay
    ...
    if review_channel:
        ...
        try:
            await review_channel.delete(reason=f"Bounty #{bounty_id} claim approved.")
...
async def deny_claim(ctx, bounty_id: int = None, *, reason: str = "No reason provided."):
    ...
    if bounty_id not in bounties or "pending_claim" not in bounties[bounty_id] or not bounties[bounty_id]["pending_claim"]:
        await ctx.send(f"❌ No pending claim found for Bounty #{bounty_id}.")
        return
