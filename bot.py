...
def load_data():
    ...
    global announced_milestones, dashboard_message_id, flip_panel_message_id, flip_hv_message_id, flip_up_message_id, flip_down_message_id, flip_guide_message_id, weekly_stats, flip_logs, ge_item_name_map
...
WIKI_VOLUMES_URL = "https://prices.runescape.wiki/api/v1/osrs/volumes"
...
MIN_VOLUME         = 50       # Minimum daily volume for Best Margins
MIN_MARGIN         = 5_000    # Minimum margin (gp) for Best Margins
HV_MIN_VOLUME      = 10_000   # Minimum daily volume for High Volume category
HV_MIN_MARGIN      = 100      # Minimum margin (gp) for High Volume category
TREND_MIN_VOLUME   = 500      # Minimum daily volume to appear in Trending
...
async def fetch_ge_prices():
    ...
    # Core fetch — latest prices + volumes (required for all categories)
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            async with session.get(WIKI_PRICES_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                prices_raw = (await r.json()).get("data", {})
            async with session.get(WIKI_VOLUMES_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                volumes_resp = await r.json()
                volumes_raw = volumes_resp.get("data", volumes_resp)
    except Exception as e:
...
async def build_flip_panel_embeds():
    ...
    prices_raw, volumes_raw, prices_24h, name_map = await fetch_ge_prices()
    DISCLAIMER = "⚠️ *Prices fluctuate — verify by buying/selling a small amount first. Use at your own risk.*"
    FOOTER     = "Data: prices.runescape.wiki • Updated every 15 min"
    now        = datetime.now()

    def safe_lines(embed, lines, sep="\n"):
        """Add lines to embed description, stopping before Discord's 4096 char limit."""
        result = []
        base_len = len(embed.description or "")
        running  = base_len + len("\n\n")
        for line in lines:
        ...
        if result:
            ...
            embed.description += "\n\n*No items matched the filters right now.*"
    ...
    margins_embed = discord.Embed(
        title="💰 Best Margins",
        description=(
            f"Top **{TOP_MARGINS_COUNT}** items by profit margin after 1% GE tax.\n"
            f"Min **{MIN_VOLUME:,}** daily trades · Min **{format_gp(MIN_MARGIN)}** margin.\n\n"
            + DISCLAIMER
        ),
        color=discord.Color.green()
    )
    if api_ok:
        ...
        for i, (name, high, low, margin, vol) in enumerate(candidates[:TOP_MARGINS_COUNT], 1):
            ...
            lines.append(
                f"`{i:>2}.` **{name}**\n"
                f"　Buy: {format_gp(low)} · Sell: {format_gp(high)} · "
                f"Margin: **{format_gp(margin)}** · ROI: {roi:.1f}% · Vol: {vol:,}/day"
            )
    ...
    hv_embed = discord.Embed(
        ...
        description=(
            f"Top **{TOP_MARGINS_COUNT}** most-traded items with a positive margin.\n"
            f"Min **{HV_MIN_VOLUME:,}** daily trades · Min **{format_gp(HV_MIN_MARGIN)}** margin.\n"
            f"Great for fast, lower-risk flipping at high quantities.\n\n"
            + DISCLAIMER
        ),
        color=discord.Color.blue()
    )
    if api_ok:
        ...
        hv_candidates.sort(key=lambda x: x[4], reverse=True)  # Sort by volume
        lines = []
    ...
    up_embed = discord.Embed(
        ...
        description=(
            f"Top **{TOP_MARGINS_COUNT}** items with the biggest price **increase** vs their 24h average.\n"
            f"Min **{TREND_MIN_VOLUME:,}** daily trades · Min price **{format_gp(TREND_MIN_PRICE)}** · "
            f"Min **{TREND_MIN_CHANGE:.0f}%** change.\n\n"
            + DISCLAIMER
        ),
    ...
    if api_ok and prices_24h:
        ...
        for i, (name, current, avg, pct, gp_chg, vol) in enumerate(up_candidates[:TOP_MARGINS_COUNT], 1):
            lines.append(
                f"`{i:>2}.` **{name}**\n"
                f"　Now: {format_gp(current)} · 24h avg: {format_gp(avg)} · "
                f"Change: **+{pct:.1f}%** (+{format_gp(gp_chg)}) · Vol: {vol:,}/day"
            )
        safe_lines(up_embed, lines)
    elif not prices_24h:
        up_embed.description += "\n\n⚠️ 24h price data unavailable."
    else:
        up_embed.description += "\n\n⚠️ Could not reach the OSRS Wiki API."
    ...
    down_embed = discord.Embed(
        ...
        description=(
            f"Top **{TOP_MARGINS_COUNT}** items with the biggest price **decrease** vs their 24h average.\n"
            f"Min **{TREND_MIN_VOLUME:,}** daily trades · Min price **{format_gp(TREND_MIN_PRICE)}** · "
            f"Min **{TREND_MIN_CHANGE:.0f}%** change.\n\n"
            + DISCLAIMER
        ),
    ...
    if api_ok and prices_24h:
        ...
        down_candidates.sort(key=lambda x: x[3])  # Most negative first
        lines = []
    ...
    guide_embed = discord.Embed(
        title="🛠️ How to Use the GE Flip Tools",
        color=discord.Color.gold(),
        description=(
            "**Price Check**\n"
            "`!price <item name>` — Live buy/sell price, margin & daily volume\n"
            "Example: `!price Dragon bones`\n\n"
            "**GE Tax Calculator**\n"
            "`!tax <sell price> [quantity]` — Calculate your tax and net proceeds\n"
            "Example: `!tax 500000 100`\n\n"
            "**Personal Flip Tracker**\n"
            "`!logflip <item> <buy> <sell> [qty]` — Log a completed flip\n"
            "`!myflips` — View your flip history and total profit\n"
            "`!clearflips` — Clear your flip history\n\n"
            "Example: `!logflip Dragon bones 2000 2500 1000`\n\n"
            "*All prices sourced from the OSRS Wiki Prices API in real time.*"
        )
    )
    guide_embed.set_footer(text="⚠️ Prices fluctuate — always verify before committing. Use at your own risk. | Avera Clan")
...
async def ge_price(ctx, *, item_name: str = None):
    ...
    embed = discord.Embed(
        title=f"💰 {result['name']}",
        color=discord.Color.green() if margin > 0 else discord.Color.red()
    )
    embed.add_field(name="📥 Instant Buy",    value=format_gp(low),    inline=True)
    embed.add_field(name="📤 Instant Sell",   value=format_gp(high),   inline=True)
    embed.add_field(name="💸 GE Tax",         value=format_gp(tax),    inline=True)
    ...
    embed.add_field(name="🔄 Daily Volume",   value=f"{vol:,}",        inline=True)
    embed.set_footer(text="Data: prices.runescape.wiki • Prices update every ~60s • Use at your own risk — verify before committing")
    embed.timestamp = datetime.now()
    await ctx.send(embed=embed)
