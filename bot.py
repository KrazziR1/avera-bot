...
async def list_bounties(ctx):
    ...
    for bounty_id, b in active:
        ...
        embed.add_field(
            name=f"{tier_info['emoji']} [{tier_info['label']}] #{bounty_id} — {b['target']}",
            value=f"💰 **{b['points']} pts + 5 ⚔️ Glory**\n📝 {b['description']}\n{status} • `!claim {bounty_id}` to claim",
            inline=False
        )
...
async def cancel_bounty(ctx, bounty_id: int = None):
    ...
    # Clean up all open review channels (claim_channels[bounty_id] is a list)
    all_channel_ids = claim_channels.pop(bounty_id, [])
    if isinstance(all_channel_ids, int):
        all_channel_ids = [all_channel_ids]  # legacy single-value safety
    for ch_id in all_channel_ids:
        review_channel = bot.get_channel(ch_id)
        if review_channel:
