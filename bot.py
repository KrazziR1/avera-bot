...
async def fetch_gear_image(wiki_name: str, style: str) -> tuple[bytes | None, str | None]:
    ...
    try:
        ...
        if target_section:
            html_params["section"] = target_section
        ...
        # Wiki item images follow pattern: /images/thumb/ItemName.png/...
        img_pattern = re.compile(
            r'src="(//oldschool\.runescape\.wiki/images(?:/thumb)?/[^"]+\.png[^"]*)"'
        )
        raw_urls = img_pattern.findall(html)

        # Clean up — remove thumb sizing params, deduplicate, filter tiny icons
        seen = set()
        item_urls = []
        for url in raw_urls:
            # Convert //domain to https://domain
            full_url = "https:" + url if url.startswith("//") else url
            # Strip thumbnail resizing — get original
            clean = re.sub(r'/thumb(/.*\.png)/\d+px-[^/]+$', r'\1', full_url)
            clean = re.sub(r'/thumb', '', clean)
            if clean not in seen and "Coins" not in clean:
                seen.add(clean)
        ...
        # Limit to first 20 items (gear slots)
