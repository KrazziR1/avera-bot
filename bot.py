...
DATA_FILE = "avera_data.json"

def load_data():
    """Load all persistent data from JSON file on startup."""
    ...
    try:
        ...
        clan_events = data.get("clan_events", [])
        bounties = {int(k): v for k, v in data.get("bounties", {}).items()}
        bounty_counter = data.get("bounty_counter", 0)
        claim_channels = {int(k): v for k, v in data.get("claim_channels", {}).items()}
        coffer_balance = data.get("coffer_balance", 0)
        coffer_donations = data.get("coffer_donations", [])
        coffer_payouts = data.get("coffer_payouts", [])
        active_giveaways = data.get("active_giveaways", [])
        verified_rsns = data.get("verified_rsns", {})
        verify_times = data.get("verify_times", {})
        member_ranks = data.get("member_ranks", {})
        referrals = data.get("referrals", {})
        event_streaks = data.get("event_streaks", {})
        shop_redemptions = data.get("shop_redemptions", [])
        announced_milestones = set(data.get("announced_milestones", []))
        dashboard_message_id = data.get("dashboard_message_id", None)
        weekly_stats.update(data.get("weekly_stats", {}))

        print(f"✅ Data loaded — {len(points_data)} members, coffer: {coffer_balance:,} gp")
...
async def dink_panel(ctx):
    ...
    embed1.add_field(
        ...
        value=(
            ...
            "1. Copy the full export string from the message below\n"
            "2. Log into OSRS\n"
            "3. In the chat box, type `::dinkimport ` *(with a space after it)* then paste the string\n"
            "4. Hit **Enter** — Dink will import everything automatically!\n\n"
    ...
    embed2 = discord.Embed(
        title="📋 Avera Dink Export String",
        description=(
            "Copy the **entire** string below and paste it into Dink's Import field:\n\n"
            f"```{export_string}```"
        ),
        color=discord.Color.blue()
    )
    embed2.set_footer(text="In-game: type ::dinkimport followed by a space, then paste the string above")
