...
async def rank_check(ctx, member: discord.Member = None):
    ...
    bar = glory_progress_bar(glory, next_threshold) if next_threshold else "█" * 10
    ...
    if next_rank:
        ...
        stats_text += f"{bar} {min(int(glory/next_threshold*100),100)}%\n\n"
    else:
...
async def profile(ctx, member: discord.Member = None):
    ...
    # Approximate total events from points awarded for attendance
    # Each attendance = 25 pts minimum, so this is a rough floor
    bounties_claimed = sum(
        1 for b in bounties.values()
...
async def verify_rsn(ctx, *, rsn: str = None):
    ...
    is_first_verify = user_id not in verify_times or old_rsn is None
