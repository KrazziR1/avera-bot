...
async def on_member_remove(member):
    ...
    if log_channel:
        ...
        embed.add_field(name="📅 Time in Clan", value=f"{months} month{'s' if months != 1 else ''}" if months > 0 else "< 1 month", inline=True)
...
def get_months_since_verify(user_id):
    ...
    # Add fractional month from days
    months += (now.day - verified_dt.day) / 30
    return max(0, months)
...
async def rank_check(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(
        ...
        description=(
            f"{stats_text}\n\n"
            + (f"✅ Rank updated to **{new_rank}**!" if role_changed else f"✔️ Already ranked as **{new_rank}**.")
        ),
        color=discord.Color.gold()
    )
    embed.set_thumbnail(url=target.display_avatar.url)
...
async def daily_rankup_check():
    ...
    if daily_rankup_check.is_running() and daily_rankup_check._task and not daily_rankup_check._task.done():
    ...
    for guild in bot.guilds:
        ...
        for user_id, rsn in list(verified_rsns.items()):
            ...
            current_role_name = next(
                (r.name for r in member.roles if r.name in RANK_EMOJIS.keys()), "Protector"
            )
...
async def rank_progress(ctx, member: discord.Member = None):
    ...
    next_level_glory = next((v for t, v in LEVEL_MILESTONES if t > (total_level or 0)), None)
    if next_level_milestone:
        suggestions.append(f"📊 Reach **{next_level_milestone:,} total level** (+{next_level_glory} Glory toward your goal)")

    # Next XP milestone
    next_xp_milestone = next((t for t, v in XP_MILESTONES if t > (total_xp or 0)), None)
    next_xp_glory = next((v for t, v in XP_MILESTONES if t > (total_xp or 0)), None)
    if next_xp_milestone:
        suggestions.append(f"🧮 Reach **{next_xp_milestone/1_000_000:.0f}M total XP** (+{next_xp_glory} Glory toward your goal)")
