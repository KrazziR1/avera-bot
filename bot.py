...
def calculate_glory(user_id, total_level=None, total_xp=None):
    ...
    if uid:
        bounties_claimed = sum(
            ...
            if not b.get("active", True) and str(b.get("claimed_by_id", "")) == uid
        )
...
def build_dashboard_embed(guild):
    ...
    for b in bounties.values():
        if not b["active"] and b.get("claimed_by_id"):
            uid = str(b["claimed_by_id"])
            bounty_counts[uid] = bounty_counts.get(uid, 0) + 1
        elif not b["active"] and b.get("claimed_by"):  # Legacy fallback
            name = b["claimed_by"]
            bounty_counts[name] = bounty_counts.get(name, 0) + 1
    if bounty_counts:
        top_hunters = sorted(bounty_counts.items(), key=lambda x: x[1], reverse=True)[:3]
        medals = ["🥇", "🥈", "🥉"]
        hunters_text = ""
        for i, (key, count) in enumerate(top_hunters):
            # key may be a user_id string or legacy display name
            if key.isdigit():
                ...
                display = key  # Legacy name string
