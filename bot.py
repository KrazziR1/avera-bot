...
async def rank_progress(ctx, member: discord.Member = None):
    ...
    # Bounties
    bounties_needed = int(needed / 5) + 1
    suggestions.append(f"🎯 Claim **{bounties_needed} more bounties** (+{bounties_needed*5:.0f} Glory)")

    # Time
    months_needed = round(needed / 15, 1)
    suggestions.append(f"⏱️ Stay active for **{months_needed} more months** (+{months_needed*15:.0f} Glory)")
...
async def add_glory(ctx, member: discord.Member = None, amount: float = None, *, reason: str = "No reason given"):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"*Run `!glory` to see their updated score.*"
        ),
        color=discord.Color.gold()
    )
    embed.set_footer(text=f"Awarded by {ctx.author.display_name}")
