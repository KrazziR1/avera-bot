...
**Ranks:**
⚔️ Protector → Bulwark → Justiciar → Sentry → Guardian → Warden → Vanguard → Templar

*Earn ranks through **Glory** — gained by time in clan, level & XP milestones, bounties, and referrals. Use `!glory` to check your score!*
"""
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        ...
        await ctx.send(f"❌ Invalid argument. Check your values and try again. Use `!help` for usage info.")
    elif isinstance(error, commands.CommandOnCooldown):
        await ctx.send(f"⏳ Slow down! Try again in {error.retry_after:.1f} seconds.", delete_after=5)
    elif isinstance(error, commands.BadArgument):
        await ctx.send(f"❌ Invalid argument. Make sure you're using the right format — use `!help` for examples.")
    elif isinstance(error, commands.CommandOnCooldown):
        ...
        if log_channel:
            ...
            msg = (
                f"⚠️ **Unhandled Error**\n"
                f"**Command:** `{ctx.command}` by {ctx.author.mention}\n"
...
def get_mongo_col():
    """Returns the MongoDB collection, creating the connection if needed. Reconnects on failure."""
    ...
    _mongo_client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000, retryWrites=True)
...
def load_data():
    ...
    global announced_milestones, dashboard_message_id, flip_panel_message_id, flip_hv_message_id, flip_up_message_id, flip_down_message_id, flip_guide_message_id, weekly_stats, flip_logs, ge_item_name_map, ge_item_limit_map, gear_cache
    ...
    try:
        ...
        glory_data.update(doc.get("glory_data", {}))
        glory_bonus.update(doc.get("glory_bonus", {}))
        wom_pending.update(doc.get("wom_pending", {}))
        loaded_attendance = doc.get("active_attendance", None)
        if loaded_attendance:
            active_attendance = loaded_attendance
        loaded_giveaways = doc.get("active_giveaways_live", {})
        active_giveaways_live.update({int(k): v for k, v in loaded_giveaways.items()})
        loaded_polls = doc.get("active_polls", {})
        active_polls.update({int(k): v for k, v in loaded_polls.items()})
        open_tickets.update({str(k): v for k, v in doc.get("open_tickets", {}).items()})
        ...
        flip_panel_message_id  = doc.get("flip_panel_message_id", None)
        flip_hv_message_id     = doc.get("flip_hv_message_id", None)
        flip_up_message_id     = doc.get("flip_up_message_id", None)
        flip_down_message_id   = doc.get("flip_down_message_id", None)
        flip_guide_message_id  = doc.get("flip_guide_message_id", None)
        weekly_stats.update(doc.get("weekly_stats", {}))
        flip_logs.update(doc.get("flip_logs", {}))
...
def save_data():
    """Save all persistent data to MongoDB. Retries once on failure."""
    for attempt in range(2):
        try:
            ...
            data = {
                ...
                "glory_data": glory_data,
                "glory_bonus": glory_bonus,
                "wom_pending": wom_pending,
                "open_tickets": open_tickets,
                "active_attendance": active_attendance,
                "active_giveaways_live": {str(k): v for k, v in active_giveaways_live.items()},
                "active_polls": {str(k): v for k, v in active_polls.items()},
                ...
                "flip_panel_message_id":  flip_panel_message_id,
                "flip_hv_message_id":     flip_hv_message_id,
                "flip_up_message_id":     flip_up_message_id,
                "flip_down_message_id":   flip_down_message_id,
                "flip_guide_message_id":  flip_guide_message_id,
            ...
            if attempt == 1:
                # Second failure — log to Discord if possible
                async def _alert_save_failure(err):
                    log_channel = bot.get_channel(LOG_CHANNEL_ID)
                    if log_channel:
                        await log_channel.send(f"🚨 **MongoDB save failed twice!** Data may be at risk.\n`{err}`")
                import asyncio
                try:
                    asyncio.get_event_loop().create_task(_alert_save_failure(e))
                except Exception:
                    pass
...
points_data = {}
flip_logs = {}  # GE flip tracker: { user_id: [ { item, buy, sell, qty, profit, timestamp } ] }
ge_item_name_map   = {}  # Cached mapping of item_id -> name (loaded once, never changes)
ge_item_limit_map  = {}  # Cached mapping of item_id -> GE 4h buy limit
gear_cache         = {}  # Boss gear cache: { "bossname_style": { data, fetched_at } }
http_session       = None  # Persistent aiohttp session — created in on_ready
...
async def on_ready():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️ Playwright install: {result.stderr[:200]}")
    except Exception as e:
    ...
    bot.add_view(FlipRefreshView())
    # Start background tasks
    if not weekly_recap.is_running():
        weekly_recap.start()
    if not flip_panel_refresh.is_running():
        flip_panel_refresh.start()
    # Pre-cache GE item name mapping (static data, only needs fetching once)
    asyncio.create_task(_cache_ge_mapping())
    ...
    if not wom_pending_check.is_running():
        wom_pending_check.start()
    # Resume any polls that were running before restart
    await resume_active_polls()
    # Resume any giveaways that were running before restart
...
async def on_member_join(member):
    ...
    channel = bot.get_channel(MEMBER_JOIN_CHANNEL_ID) or \
    ...
    if channel:
        embed = discord.Embed(
            ...
            description=(
                f"Hey {member.mention}, welcome to **{CLAN_NAME}**! 🎉\n\n"
                f"**1️⃣ Verify your RSN**\n"
                f"Head to <#{WELCOME_PANEL_CHANNEL_ID}> and use `!verify <username>` to link your OSRS account and unlock the server.\n\n"
                f"**2️⃣ Claim your rank**\n"
                f"Run `!rankcheck` to get your Discord rank assigned based on your in-game stats. Ranks in Avera are earned through **Glory** — a permanent score built from your time in the clan, your levels, XP milestones, and more. Use `!glory` anytime to see your full breakdown. The more bounties you complete in <#{BOUNTY_CHANNEL_ID}>, the faster you'll climb! 🎯\n\n"
                f"**3️⃣ Set up Dink (optional)**\n"
                f"Want your drops, level ups, and achievements automatically posted in <#{DINK_CHANNEL_ID}>? Visit <#{DINK_CHANNEL_ID}> for the setup guide.\n\n"
                f"**4️⃣ Join the CC**\n"
                f"Head to <#{TICKET_PANEL_CHANNEL_ID}> and open a **⚔️ Join Clan** ticket — no interview or application needed, it just lets staff know you're ready to be added.\n\n"
                f"**5️⃣ Join \"Avera\" in-game**\n"
                f"Search **\"Avera\"** in-game and join the CC if you haven't already!\n\n"
                f"**6️⃣ Explore Avera**\n"
                f"📋 <#{FAQ_CHANNEL_ID}> — answers to everything\n"
                f"🏰 <#{DASHBOARD_CHANNEL_ID}> — clan stats, events & giveaways\n"
                f"🛒 <#1479472183221682298> — spend your points on clan rewards\n\n"
...
async def on_member_remove(member):
    """Logs when a member leaves and notifies the log channel."""
    user_id = str(member.id)
    rsn = verified_rsns.get(user_id, "Not verified")
    points = points_data.get(user_id, 0)
    cached_glory = glory_data.get(user_id, {}).get("glory", 0)
    ...
    if log_channel:
        ...
        if months_int == 0:
            months_display = "< 1 month"
        elif months_int == 1:
            months_display = "1 month"
        else:
            months_display = f"{months_int} months"
        ...
        embed.add_field(name="⚔️ Glory", value=f"{cached_glory:.1f}", inline=True)
        embed.add_field(name="🏅 Points", value=f"{points:,}", inline=True)
        embed.add_field(name="📅 Time in Clan", value=months_display, inline=True)
        embed.set_footer(text="Their data has been retained in case they rejoin.")
...
async def help_command(ctx):
    ...
    embed.add_field(name="📅 Events", value=(
        ...
        f"`{PREFIX}removeevent <number>` — Remove an event *(Admin only)*\n"
    ...
    embed.add_field(name="🏆 Points & Ranks", value=(
        ...
        f"`{PREFIX}removepoints @member <amount>` — Remove points *(Admin only)*\n"
        ...
        f"`{PREFIX}glory [@member]` — View Glory score and full breakdown\n"
        f"`{PREFIX}rankprogress [@member]` — See exactly what you need for your next rank\n"
        f"`{PREFIX}assignrank @member Moderator` — Assign Moderator rank *(Owner only)*\n"
        f"`{PREFIX}removerank @member` — Remove admin rank *(Owner only)*\n"
    ...
    embed.add_field(name="🎯 Bounties", value=(
        ...
        f"`{PREFIX}reopenbounty <id>` — Reopen a closed bounty *(Admin only)*"
    ), inline=False)
    ...
    embed.add_field(name="🛒 Clan Shop", value=(
        ...
        f"`{PREFIX}shoppanel` — Post a permanent shop panel *(Admin only)*\n"
        f"`{PREFIX}redemptions` — View pending redemptions *(Admin only)*\n"
        f"`{PREFIX}fulfil <#>` — Mark a redemption as done *(Admin only)*"
    ), inline=False)
    embed.add_field(name="⚔️ Boss Guides", value=(
        f"`{PREFIX}gear <boss>` — Full gear setup, inventory, prayers & settings for any boss\n"
        f"` ` Example: `!gear Zulrah` — choose Magic/Ranged/Melee via buttons\n"
        f"`{PREFIX}gear Vorkath` · `!gear Cerberus` · `!gear Nex` — works for all bosses"
    ), inline=False)
    embed.add_field(name="💰 GE Flipping", value=(
        f"`{PREFIX}price <item>` — Live buy/sell price, suggested offers & 6h volume\n"
        f"`{PREFIX}tax <sell price> [qty]` — Calculate GE tax and net proceeds\n"
        f"`{PREFIX}logflip <item> <buy> <sell> [qty]` — Log a completed flip\n"
        f"`{PREFIX}myflips [@member]` — View flip history and total profit\n"
        f"`{PREFIX}clearflips` — Clear your personal flip log\n"
        f"`{PREFIX}flippanel` — Repost the live GE flip panel *(Admin only)*"
    ), inline=False)
    ...
    embed.add_field(name="🔥 Streaks & Verify", value=(
        ...
        f"`{PREFIX}setverifydate @member YYYY-MM-DD` — Backdate a member's verify date *(Admin only)*\n"
        f"`{PREFIX}addglory @member <amount> [reason]` — Award bonus Glory to a member *(Admin only)*\n"
        f"`{PREFIX}womcheck` — Check which verified members are missing from WOM *(Admin only)*\n"
        f"`{PREFIX}womclear @member` — Dismiss a WOM pending alert *(Admin only)*\n"
        f"`{PREFIX}removeglory @member <amount> [reason]` — Remove bonus Glory from a member *(Admin only)*\n"
        f"`{PREFIX}hiscoresdebug <rsn>` — Debug raw hiscores data *(Admin only)*"
    ), inline=False)
...
async def stats(ctx, *, username: str = None):
    ...
    try:
        session = http_session  # persistent session
...
active_giveaways_live = {}  # { message_id: { prize, host_id, end_time, channel_id } } — declared early for persistence
...
async def attend(ctx):
    ...
    if user_id not in verified_rsns:
        await ctx.send(f"❌ {ctx.author.mention} you need to verify your RSN first! Use `!verify <username>` to link your account.")
        return
    ...
    rank = get_rank_display(get_rank_from_glory(glory_data.get(user_id, {}).get("glory", 0), user_id))
...
PLACEMENT_POINTS = {1: 50, 2: 25, 3: 10}
...
async def leaderboard(ctx):
    ...
    leaderboard_text = ""
    rank = 0
    for user_id, pts in sorted_members:
        member = ctx.guild.get_member(int(user_id))
        if not member:
            # Show with RSN if we have it, skip fully unknown entries
            rsn = verified_rsns.get(user_id)
            if not rsn:
                continue
            name = f"{rsn} *(left server)*"
        else:
    ...
    embed.description = leaderboard_text or "*No points awarded yet.*"
    embed.set_footer(text="Avera Clan Points System")
    await ctx.send(embed=embed)
...
async def clan_stats(ctx):
    ...
    # Rank distribution — based on cached glory
    rank_counts = {r: 0 for r in RANK_EMOJIS.keys()}
    for uid in verified_rsns.keys():
        cached = glory_data.get(uid, {}).get("glory", 0)
        rank = get_rank_from_glory(cached, uid)
...
# Custom emoji IDs
RANK_EMOJIS = {
    "Protector":  "<:Protector:1480321151044423905>",
    "Bulwark":    "<:Bulwark:1480321160976531551>",
    "Justiciar":  "<:Justiciar:1480321170091016192>",
    "Sentry":     "<:Sentry:1480321179485995099>",
    "Guardian":   "<:Guardian:1480321186977157171>",
    "Warden":     "<:Warden:1480321195927801996>",
    "Vanguard":   "<:Vanguard:1480321205293809684>",
    "Templar":    "<:Templar:1480321213652795473>",
}

# Glory rank thresholds (auto-earned)
GLORY_THRESHOLDS = [
    ("Templar",   1050),
    ("Vanguard",   820),
    ("Warden",     620),
...
# Level milestone Glory values (cumulative)
LEVEL_MILESTONES = [
    (500,  20),
    (750,  30),
    (1000, 50),
...
# XP milestone Glory values (cumulative)
XP_MILESTONES = [
    (50_000_000,    15),
    (100_000_000,   25),
    (200_000_000,   40),
...
# Staff rank (owner assigned only)
ADMIN_RANKS = ["Moderator"]

# User IDs excluded from public rank-up announcements and DMs (e.g. staff/bot testers)
# Their glory and rank still updates normally — they just won't get announced
RANK_UP_EXCLUDE = {
    # Add Discord user IDs here as integers, e.g: 123456789012345678
}
...
def get_months_since_verify(user_id):
    """Returns fractional months since a member verified."""
    ts = verify_times.get(str(user_id))
    if not ts:
        return 0
    verified_dt = datetime.fromisoformat(ts)
    now = datetime.now()
    # Use total days elapsed for accurate fractional months (avoids negative day fractions)
    delta_days = (now - verified_dt).days
    return max(0, delta_days / 30.44)  # 30.44 = average days per month


def get_level_glory(total_level):
    """Returns cumulative Glory from level milestones."""
    if not total_level:
        return 0
    glory = 0
    for threshold, value in LEVEL_MILESTONES:
...
def get_xp_glory(total_xp):
    """Returns cumulative Glory from XP milestones."""
    if not total_xp:
        return 0
    glory = 0
    for threshold, value in XP_MILESTONES:
...
def calculate_glory(user_id, total_level=None, total_xp=None):
    """Calculate a member's total Glory score from all inputs."""
    uid = str(user_id) if user_id else None
    glory = 0.0

    # Time in clan: 15 pts per month
    months = get_months_since_verify(uid) if uid else 0
    glory += months * 15

    # Total level: milestone-based
    glory += get_level_glory(total_level)

    # Total XP: milestone-based
    glory += get_xp_glory(total_xp)

    # Bounties claimed: 5 pts each
    # Also count legacy bounties stored by display name (before claimed_by_id was added)
    if uid:
        # Resolve display names for legacy bounty matching
        uid_display_names = set()
        for guild in bot.guilds:
            m = guild.get_member(int(uid))
            if m:
                uid_display_names.add(m.display_name)
        rsn_name = verified_rsns.get(uid)
        if rsn_name:
            uid_display_names.add(rsn_name)

        bounties_claimed = sum(
            1 for b in bounties.values()
            if not b.get("active", True) and (
                str(b.get("claimed_by_id", "")) == uid or
                (not b.get("claimed_by_id") and b.get("claimed_by") in uid_display_names)
            )
        )
        glory += bounties_claimed * 5

    # Referrals: 10 pts each
    if uid:
        ref_count = sum(1 for v in referrals.values() if v == uid)
        glory += ref_count * 10

    # Manual glory bonus (persists across recalculations)
    if uid:
        glory += glory_bonus.get(uid, 0)

    return round(glory, 2)


def get_rank_from_glory(glory, user_id=None):
    """Returns rank name based on Glory score, with admin override."""
    uid = str(user_id) if user_id else None
    if uid and uid in member_ranks:
        return member_ranks[uid]
    for rank_name, threshold in GLORY_THRESHOLDS:
        if glory >= threshold:
            return rank_name
    return "Protector"


def get_rank(points, user_id=None, total_level=None, total_xp=None):
    """Wrapper — calculates Glory and returns rank. Keeps old signature compatible."""
    uid = str(user_id) if user_id else None
    if uid and uid in member_ranks:
        return member_ranks[uid]
    glory = calculate_glory(uid, total_level, total_xp)
    return get_rank_from_glory(glory, uid)


def get_next_rank(current_rank):
    """Returns the next rank above current, or None if at top."""
    rank_order = [r for r, _ in reversed(GLORY_THRESHOLDS)]
    try:
...
def get_glory_for_rank(rank_name):
    """Returns the Glory threshold for a rank."""
    for r, t in GLORY_THRESHOLDS:
        if r == rank_name:
            return t
    return 0
...
def glory_progress_bar(current, target, length=10):
...
async def fetch_wom_members():
    """Fetch all member RSNs from the WOM group. Returns a list of lowercase RSNs."""
    try:
        url = f"https://api.wiseoldman.net/v2/groups/{WOM_GROUP_ID}/members"
        headers = {
            "User-Agent": "Avera-Clan-Bot",
        ...
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as resp:
            if resp.status == 200:
                ...
                return [m["player"]["displayName"].lower().replace(" ", "_") for m in members if "player" in m]
            elif resp.status == 429:
                print("⚠️ WOM API rate limited — will retry next cycle")
                return None
            else:
                text = await resp.text()
                print(f"⚠️ WOM API returned status {resp.status}: {text[:200]}")
                return None
    except asyncio.TimeoutError:
        print("⚠️ WOM fetch timed out")
        return None
    except Exception as e:
        print(f"⚠️ WOM fetch failed: {e}")
    return None


async def is_player_in_wom_group(rsn: str) -> bool:
    """Check if a single RSN is in the WOM group by looking up their groups. Returns True/False/None on error."""
    try:
        encoded = rsn.replace(" ", "%20")
        url = f"https://api.wiseoldman.net/v2/players/{encoded}/groups"
        headers = {
            "User-Agent": "Avera-Clan-Bot",
            "Content-Type": "application/json"
        }
        ...
        async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status == 200:
                data = await resp.json()
                for entry in data:
                    if entry.get("group", {}).get("id") == WOM_GROUP_ID:
                        return True
                return False
            elif resp.status == 404:
                return False  # Player not tracked on WOM at all
            else:
                return None  # Unknown error, don't penalise
    except Exception:
        return None


def rsn_in_wom(rsn, wom_members):
    """Fuzzy match RSN against WOM member list — handles spaces vs underscores and case."""
    if not rsn or not wom_members:
        return False
    normalised = rsn.lower().replace(" ", "_")
    return normalised in wom_members


async def fetch_osrs_stats(rsn):
    """Fetch total level and total XP from OSRS hiscores. Returns (total_level, total_xp)."""
    total_level = None
    total_xp = None
    try:
        ...
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
            if resp.status == 200:
                ...
                total_xp = int(overall[2]) if len(overall) > 2 else None
    except Exception:
        pass
...
async def assign_rank_role(guild, member, rank_name):
    ...
    if not rank_role:
        # Warn in log channel instead of silently creating a misconfigured role
        log_channel = bot.get_channel(LOG_CHANNEL_ID)
        if log_channel:
            await log_channel.send(
                f"⚠️ **Missing rank role:** `{rank_name}` does not exist in the server. "
                f"Please create it manually. Could not assign rank to {member.mention}."
            )
        return False
...
async def rank_check(ctx, member: discord.Member = None):
    ...
    if ctx.channel.id == WELCOME_PANEL_CHANNEL_ID:
        msg = await ctx.send(f"❌ Please use `!rankcheck` outside of this channel!")
        await asyncio.sleep(7)
        try:
            await msg.delete()
        except discord.NotFound:
    ...
    total_level, total_xp = await fetch_osrs_stats(rsn)
    if total_level is None or total_xp is None:
        await msg.edit(content=f"⚠️ Could not reach the OSRS hiscores for **{rsn}** right now. Please try again in a moment.")
        return
    months = get_months_since_verify(user_id)
    glory = calculate_glory(user_id, total_level, total_xp)

    # Cache glory
    glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
    save_data()

    new_rank = get_rank_from_glory(glory, user_id)
    rank_display = get_rank_display(new_rank)
    next_rank = get_next_rank(new_rank)
    next_threshold = get_glory_for_rank(next_rank) if next_rank else None
    current_threshold = get_glory_for_rank(new_rank)
    bar = glory_progress_bar(glory - current_threshold, next_threshold - current_threshold if next_threshold else 1) if next_threshold else "█" * 10

    role_changed = await assign_rank_role(ctx.guild, target, new_rank)

    stats_text = f"⚔️ **Glory:** {glory:.1f}\n"
    if next_rank:
        needed = max(0, next_threshold - glory)
        tier_progress = max(0, glory - current_threshold)
        tier_total = next_threshold - current_threshold
        pct = min(int(tier_progress / tier_total * 100), 100) if tier_total > 0 else 100
        stats_text += f"📈 **Next Rank:** {get_rank_display(next_rank)} ({needed:.1f} Glory needed)\n"
        stats_text += f"{bar} {pct}%\n\n"
    else:
        stats_text += f"🏆 **Max rank achieved!**\n{bar}\n\n"
    if total_level:
        stats_text += f"📊 **Total Level:** {total_level:,}\n"
    if total_xp:
        stats_text += f"🧮 **Total XP:** {total_xp/1_000_000:.0f}M\n"
    stats_text += f"📅 **Months Verified:** {months:.1f}"

    # Determine if rank actually changed by comparing before/after roles
    current_role_name = next(
        (r.name for r in target.roles if r.name in RANK_EMOJIS.keys()), None
    )
    rank_changed = role_changed or (current_role_name != new_rank)

    embed = discord.Embed(
        title=f"{rank_display} — {target.display_name}",
        description=(
            f"{stats_text}\n\n"
            + (f"✅ Rank updated to **{new_rank}**!" if rank_changed else f"✔️ Already ranked as **{new_rank}**.")
        ),
        color=discord.Color.gold()
    )
    embed.set_thumbnail(url=target.display_avatar.url)
...
async def assign_rank(ctx, member: discord.Member = None, *, rank: str = None):
    ...
    valid_admin_ranks = ADMIN_RANKS
    matched = next((r for r in valid_admin_ranks if r.lower() == rank.lower()), None)
    if not matched:
        await ctx.send(f"❌ Unknown admin rank **{rank}**. Valid admin ranks: {', '.join(valid_admin_ranks)}")
        return
...
async def remove_rank(ctx, member: discord.Member = None):
    ...
    if user_id in member_ranks:
        ...
        # Immediately reassign the correct glory-based rank
        rsn = verified_rsns.get(user_id)
        if rsn:
            total_level, total_xp = await fetch_osrs_stats(rsn)
            if total_level is not None and total_xp is not None:
                glory = calculate_glory(user_id, total_level, total_xp)
                glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
                new_rank = get_rank_from_glory(glory, user_id)
                await assign_rank_role(ctx.guild, member, new_rank)
                save_data()
                await ctx.send(f"✅ Removed admin-assigned rank from **{member.display_name}**. They have been reassigned to **{get_rank_display(new_rank)}** based on their Glory.")
                return
        await ctx.send(f"✅ Removed admin-assigned rank from **{member.display_name}**. Run `!rankcheck` on them to assign their glory-based rank.")
    else:
...
async def rank_update_all(ctx):
    ...
    for user_id, rsn in verified_rsns.items():
        ...
        # Skip admin-ranked members — their rank is manually assigned
        if user_id in member_ranks:
            skipped += 1
            continue

        total_level, total_xp = await fetch_osrs_stats(rsn)
        await asyncio.sleep(0.6)  # Unconditional delay — prevents hiscores rate limiting
        if total_level is None or total_xp is None:
            hiscores_failed += 1
            continue  # Don't overwrite with bad data
    ...
    summary = f"✅ Rank update complete! **{updated}** members updated, **{skipped}** skipped."
    if hiscores_failed > 0:
        summary += f"\n⚠️ **{hiscores_failed}** members skipped due to hiscores being unavailable — re-run to retry."
    await msg.edit(content=summary)


# ── Daily Rank-Up Notification Task ──────────────────────────

_daily_rankup_running  = False
_wom_pending_running   = False
_wom_check_running    = False

@tasks.loop(hours=24)
async def daily_rankup_check():
    """Once a day, check all verified members for rank ups using Glory system."""
    global _daily_rankup_running
    if _daily_rankup_running:
        return  # Previous run still in progress — skip this cycle
    _daily_rankup_running = True
    try:
        for guild in bot.guilds:
            ...
            for user_id, rsn in list(verified_rsns.items()):
                ...
                if user_id in RANK_UP_EXCLUDE:
                    continue

                current_role_name = next(
                    (r.name for r in member.roles if r.name in RANK_EMOJIS.keys()), None
                )
                # If member has no rank role yet, assign their correct rank without announcing
                ...
                total_level, total_xp = await fetch_osrs_stats(rsn)
                await asyncio.sleep(0.6)  # Unconditional delay — avoids hiscores rate limits
                if total_level is None or total_xp is None:
                    continue  # Skip this member if hiscores are down — don't overwrite with bad data
                glory = calculate_glory(user_id, total_level, total_xp)
                glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
                ...
                if new_idx > current_idx:
                    ...
                    announce_embed = discord.Embed(
                        ...
                        description=(
                            ...
                            f"You\'ve earned the rank of **{get_rank_display(new_rank)}** in **Avera**!\n"
                            f"**Glory:** {glory:.1f} ⚔️\n\n"
                            f"Keep grinding — the clan is proud of you! 🏆"
                        ),
                        color=discord.Color.gold()
                    )
                    announce_embed.set_thumbnail(url=member.display_avatar.url)
                    announce_embed.set_footer(text="Avera Clan • Glory Rank System")
                    if announcements:
                        await announcements.send(embed=announce_embed)
...
@bot.command(name="glory")
async def glory_command(ctx, member: discord.Member = None):
    """View your Glory score and rank progress. Usage: !glory [@member]"""
    target = member or ctx.author
    user_id = str(target.id)
    rsn = verified_rsns.get(user_id)

    if not rsn:
        await ctx.send(f"❌ **{target.display_name}** hasn\'t verified their RSN yet.")
        return

    msg = await ctx.send(f"⚔️ Calculating Glory for **{target.display_name}**...")

    total_level, total_xp = await fetch_osrs_stats(rsn)
    hiscores_failed = total_level is None or total_xp is None
    total_level = total_level or 0
    total_xp = total_xp or 0
    months = get_months_since_verify(user_id)
    glory = calculate_glory(user_id, total_level, total_xp)

    # Cache glory and update rank role
    glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
    new_rank = get_rank_from_glory(glory, user_id)
    if ctx.guild:
        await assign_rank_role(ctx.guild, target, new_rank)
    save_data()

    current_rank = new_rank
    next_rank = get_next_rank(current_rank)
    next_threshold = get_glory_for_rank(next_rank) if next_rank else None
    current_threshold = get_glory_for_rank(current_rank)
    bar = glory_progress_bar(glory - current_threshold, next_threshold - current_threshold if next_threshold else 1) if next_threshold else "█" * 10

    # Build breakdown
    time_glory = round(months * 15, 1)
    level_glory = get_level_glory(total_level)
    xp_glory = get_xp_glory(total_xp)
    bounties_claimed = sum(1 for b in bounties.values() if not b.get("active", True) and str(b.get("claimed_by_id", "")) == user_id)
    bounty_glory = bounties_claimed * 5
    ref_count = sum(1 for v in referrals.values() if v == user_id)
    ref_glory = ref_count * 10
    bonus_glory = glory_bonus.get(user_id, 0)

    # Next level and XP milestones
    next_level_milestone = next((t for t, v in LEVEL_MILESTONES if t > (total_level or 0)), None)
    next_level_glory = next((v for t, v in LEVEL_MILESTONES if t > (total_level or 0)), None)
    next_xp_milestone = next((t for t, v in XP_MILESTONES if t > (total_xp or 0)), None)
    next_xp_glory = next((v for t, v in XP_MILESTONES if t > (total_xp or 0)), None)

    embed = discord.Embed(
        title=f"⚔️ {target.display_name}\'s Glory",
        color=discord.Color.gold()
    )
    embed.set_thumbnail(url=target.display_avatar.url)

    embed.add_field(
        name=f"{get_rank_display(current_rank)}",
        value=(
            f"**Total Glory: {glory:.1f}**\n"
            + (f"{bar} → {get_rank_display(next_rank)} ({max(0, next_threshold - glory):.1f} needed)\n" if next_rank else f"{bar} 🏆 **Max rank!**\n")
        ),
        inline=False
    )

    level_text = f"📊 **Total Level** ({total_level or 0:,}): **+{level_glory}**"
    if next_level_milestone:
        level_text += f" *(next milestone: {next_level_milestone:,} = +{next_level_glory})*"

    xp_text = f"🧮 **Total XP** ({(total_xp or 0)/1_000_000:.0f}M): **+{xp_glory}**"
    if next_xp_milestone:
        xp_text += f" *(next milestone: {next_xp_milestone/1_000_000:.0f}M = +{next_xp_glory})*"

    embed.add_field(
        name="📊 Glory Breakdown",
        value=(
            f"⏱️ Time in clan ({months:.1f} months): **+{time_glory}**\n"
            f"{level_text}\n"
            f"{xp_text}\n"
            f"🎯 Bounties claimed ({bounties_claimed}): **+{bounty_glory}**\n"
            f"👥 Referrals ({ref_count}): **+{ref_glory}**"
            + (f"\n🎁 Bonus Glory: **+{bonus_glory:.0f}**" if bonus_glory > 0 else "")
        ),
        inline=False
    )

    if hiscores_failed:
        embed.set_footer(text="⚠️ Hiscores unavailable — level/XP shown as 0. Try again shortly.")
    else:
        embed.set_footer(text="Glory is permanent and never decreases • Use !rankprogress for next rank tips")
    await msg.edit(content=None, embed=embed)


@bot.command(name="rankprogress")
async def rank_progress(ctx, member: discord.Member = None):
    """See exactly what you need for your next rank. Usage: !rankprogress [@member]"""
    target = member or ctx.author
    user_id = str(target.id)
    rsn = verified_rsns.get(user_id)
    ...
    msg = await ctx.send(f"📈 Calculating rank progress for **{target.display_name}**...")
    ...
    current_rank = get_rank_from_glory(glory, user_id)
    next_rank = get_next_rank(current_rank)

    if not next_rank:
        embed = discord.Embed(
            title=f"🏆 {target.display_name} — Max Rank!",
            description=f"You\'ve reached **{get_rank_display(current_rank)}** — the highest earnable rank in Avera. Legendary! ⚔️",
            color=discord.Color.gold()
        )
        await msg.edit(content=None, embed=embed)
        return

    next_threshold = get_glory_for_rank(next_rank)
    needed = max(0, next_threshold - glory)
    current_threshold = get_glory_for_rank(current_rank)
    bar = glory_progress_bar(glory - current_threshold, next_threshold - current_threshold)
    ...
    # Bounties
    bounties_needed = max(1, -(-int(needed) // 5))  # ceiling division
    suggestions.append(f"🎯 Claim **{bounties_needed} more bounties** (+{bounties_needed*5:.0f} Glory)")

    # Time
    months_needed = round(needed / 15, 1)
    suggestions.append(f"⏱️ **{months_needed} more months** of clan membership will passively add +{months_needed*15:.0f} Glory")

    # Next level milestone — show incremental gain (difference from current milestone)
    current_level_glory = get_level_glory(total_level or 0)
    next_level_milestone = next((t for t, v in LEVEL_MILESTONES if t > (total_level or 0)), None)
    next_level_glory = next((v for t, v in LEVEL_MILESTONES if t > (total_level or 0)), None)
    if next_level_milestone and next_level_glory is not None:
        level_glory_gain = next_level_glory - current_level_glory
        suggestions.append(f"📊 Reach **{next_level_milestone:,} total level** (+{level_glory_gain} Glory)")

    # Next XP milestone — show incremental gain
    current_xp_glory = get_xp_glory(total_xp or 0)
    next_xp_milestone = next((t for t, v in XP_MILESTONES if t > (total_xp or 0)), None)
    next_xp_glory = next((v for t, v in XP_MILESTONES if t > (total_xp or 0)), None)
    if next_xp_milestone and next_xp_glory is not None:
        xp_glory_gain = next_xp_glory - current_xp_glory
        suggestions.append(f"🧮 Reach **{next_xp_milestone/1_000_000:.0f}M total XP** (+{xp_glory_gain} Glory)")

    # Referrals — cap at 3 max
    refs_needed = min(3, max(1, int(needed / 10)))
    suggestions.append(f"👥 Refer **{refs_needed} new member(s)** (+{refs_needed*10:.0f} Glory)")

    embed = discord.Embed(
        title=f"📈 {target.display_name} — Path to {get_rank_display(next_rank)}",
        color=discord.Color.blue()
    )
    embed.set_thumbnail(url=target.display_avatar.url)
    embed.add_field(
        name="Progress",
        value=f"{get_rank_display(current_rank)} {bar} {get_rank_display(next_rank)}\n**{glory:.1f} / {next_threshold} Glory** ({needed:.1f} needed)",
        inline=False
    )
    embed.add_field(
        name="💡 Ways to earn the Glory you need",
        value="\n".join(suggestions),
        inline=False
    )
    # Cache the freshly calculated glory
    glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
    save_data()

    embed.set_footer(text="Glory is permanent — every bit you earn counts forever")
    await msg.edit(content=None, embed=embed)
...
async def profile(ctx, member: discord.Member = None):
    ...
    # Use live Glory if RSN is available, otherwise fall back to cache
    hiscores_ok = True
    if rsn:
        total_level, total_xp = await fetch_osrs_stats(rsn)
        if total_level is None or total_xp is None:
            # Hiscores failed — fall back to cache
            hiscores_ok = False
            cached_glory = glory_data.get(user_id, {}).get("glory", 0)
            total_level, total_xp = 0, 0
        else:
            cached_glory = calculate_glory(user_id, total_level, total_xp)
            glory_data[user_id] = {"glory": cached_glory, "last_updated": datetime.now().isoformat()}
            save_data()
    else:
        cached_glory = glory_data.get(user_id, {}).get("glory", 0)
        total_level, total_xp = 0, 0
    current_rank = get_rank_from_glory(cached_glory, user_id)
    next_rank = get_next_rank(current_rank)
    next_threshold = get_glory_for_rank(next_rank) if next_rank else None
    current_threshold = get_glory_for_rank(current_rank)
    rank_display = get_rank_display(current_rank)

    # Count bounties using claimed_by_id (reliable), fall back to display name for legacy entries
    bounties_claimed = sum(
        1 for b in bounties.values()
        if not b.get("active", True) and (
            str(b.get("claimed_by_id", "")) == user_id or
            (not b.get("claimed_by_id") and b.get("claimed_by") == target.display_name)
        )
    )

    # Shop redemptions
    redemptions = sum(1 for r in shop_redemptions if r.get("buyer_id") == user_id)

    # Glory progress
    bar = glory_progress_bar(cached_glory - get_glory_for_rank(current_rank),
                             next_threshold - get_glory_for_rank(current_rank) if next_threshold else 1) if next_threshold else "█" * 10
    ...
    embed.add_field(name="🎮 RSN", value=rsn if rsn else "*Not verified*", inline=True)
    ...
    embed.add_field(name="📅 Time in Clan", value=months_display, inline=True)
    embed.add_field(name="⚔️ Glory", value=f"{cached_glory:.1f}", inline=True)
    embed.add_field(name="🏅 Clan Points", value=f"{points:,} pts", inline=True)
    embed.add_field(name="🔥 Event Streak", value=f"{streak} consecutive event{'s' if streak != 1 else ''}", inline=True)
    embed.add_field(name="🎯 Bounties Claimed", value=str(bounties_claimed), inline=True)
    embed.add_field(name="🛒 Shop Redemptions", value=str(redemptions), inline=True)

    if next_rank:
        needed = max(0, next_threshold - cached_glory)
        embed.add_field(
            name=f"⬆️ Next Rank — {get_rank_display(next_rank)}",
            value=f"{bar}\n**{cached_glory:.1f} / {next_threshold} Glory** ({needed:.1f} needed)\nUse `!rankprogress` for tips on earning Glory faster!",
            inline=False
        )
    else:
        embed.add_field(name="🏆 Max Rank", value="You've reached the highest earnable rank. Legendary! ⚔️", inline=False)

    footer = "Avera Clan • Use !glory for full breakdown • !rankprogress for next rank tips"
    embed.set_footer(text=footer)
    if not hiscores_ok:
        embed.add_field(
            name="⚠️ Hiscores Unavailable",
            value="Could not reach the OSRS hiscores right now. Glory and stats shown are from the last successful fetch and may be outdated.",
            inline=False
        )
    embed.timestamp = datetime.now()
    await ctx.send(embed=embed)
...
async def compare(ctx, member1: discord.Member = None, member2: discord.Member = None):
    ...
    for target in [member1, member2]:
        ...
        total_level, total_xp = await fetch_osrs_stats(rsn) if rsn else (None, None)
        glory = calculate_glory(uid, total_level, total_xp)
        rank = get_rank_from_glory(glory, uid)
    ...
    xp_w = winner(a["total_xp"], b_data["total_xp"])
...
claim_channels = {}
glory_data = {}  # { user_id: {"glory": float, "last_updated": iso_str} }
wom_pending = {}  # { user_id: iso_str } — members notified about missing WOM membership, timestamp of notification
glory_bonus = {}  # { user_id: float } — manual bonus, persists across recalculations
...
BOUNTY_TIERS = {
    "Bronze":   {"min": 1,   "max": 25,  "emoji": "🥉", "color": 0xCD7F32, "label": "Bronze"},
    "Silver":   {"min": 26,  "max": 35,  "emoji": "🥈", "color": 0xC0C0C0, "label": "Silver"},
    "Gold":     {"min": 36,  "max": 50,  "emoji": "🥇", "color": 0xFFD700, "label": "Gold"},
    "Infernal": {"min": 51,  "max": 9999,"emoji": "🔥", "color": 0xFF4500, "label": "Infernal"},
}

def get_bounty_tier(points):
    """Returns tier dict based on point value."""
    for tier in BOUNTY_TIERS.values():
        if tier["min"] <= points <= tier["max"]:
            return tier
    return BOUNTY_TIERS["Infernal"]
...
async def create_bounty(ctx, target: str = None, points: int = None, *, description: str = None):
    ...
    embed = discord.Embed(
        title=f"{tier['emoji']} [{tier['label'].upper()} BOUNTY] — {target}!",
        description=(
            f"There is a new **{tier['label']}** bounty on **{target}**!\n\n"
            f"📝 {description}\n\n"
            f"Use `!claim {bounty_id}` and send proof to be rewarded **{points} points** and **+5 Glory**!"
        ),
        color=discord.Color(tier["color"])
    )
    embed.add_field(name="🏆 Reward", value=f"{points} pts + 5 ⚔️ Glory", inline=True)
    ...
    embed.set_footer(text=f"Use !claim {bounty_id} to claim")
    embed.timestamp = datetime.now()
...
async def claim_bounty(ctx, bounty_id: int = None):
    ...
    if str(ctx.author.id) not in verified_rsns:
        await ctx.send("❌ You need to verify your RSN before claiming a bounty! Use `!verify <username>` to link your account.")
        return
    ...
    # Prevent the same user from submitting multiple claims on the same bounty
    existing_claims = bounty.get("pending_claims", [])
    if any(str(c.get("user_id")) == str(ctx.author.id) for c in existing_claims):
        await ctx.send(f"❌ You already have a pending claim on Bounty #{bounty_id}! Wait for an admin to review it.")
        return
    ...
    if not ctx.message.attachments:
        await ctx.send(
            ...
            f"📸 **Screenshot tips:**\n"
            f"• Make sure your screenshot shows the **date and time** (visible in-game clock or OS clock)\n"
            f"• Or type **`bounty #{bounty_id}`** in your in-game chat before taking the screenshot so it's visible\n"
            f"• This helps admins verify the kill was recent and legitimate 🎯\n\n"
            f"Re-run `!claim {bounty_id}` and attach your screenshot!"
        )
        return
    ...
    claim_tier = BOUNTY_TIERS.get(bounty.get("tier", get_bounty_tier(bounty["points"])["label"]), get_bounty_tier(bounty["points"]))
    embed = discord.Embed(
        title=f"⏳ {claim_tier['emoji']} [{claim_tier['label']}] Bounty #{bounty_id} Claim — Awaiting Review",
        description=(
            f"Hey {ctx.author.mention}! Your claim has been received.\n\n"
            f"🎯 **Target:** {bounty['target']}\n"
            f"🏆 **Reward:** {bounty['points']} pts + 5 ⚔️ Glory\n"
            f"{claim_tier['emoji']} **Tier:** {claim_tier['label']}\n\n"
            f"⏰ **Timestamp reminder:** If your screenshot doesn't show a visible date/time, "
            f"an admin may ask you to resubmit. For future claims, type **`bounty {bounty_id}`** "
            f"in your in-game chat before screenshotting, e.g. `bounty #{bounty_id}`, so the ID and timestamp are both clearly visible!\n\n"
        ...
        color=discord.Color(claim_tier["color"])
    )
    ...
    await review_channel.send(
        embed=embed,
        content="⚠️ **Staff note:** Discord attachment URLs expire. If reviewing this later, download the screenshot above before approving."
    )
    save_data()  # Persist pending claim in case of restart
...
async def approve_claim(ctx, bounty_id: int = None):
    ...
    # Resolve claimer from stored user_id (safe across restarts)
    claimer_id = matched_claim.get("user_id") or (matched_claim.get("user") and matched_claim["user"].id)
    claimer = ctx.guild.get_member(int(claimer_id)) if claimer_id else None
    claimer_name = matched_claim.get("user_name", "Unknown")
    if not claimer:
        await ctx.send(f"⚠️ Could not find the member who submitted this claim. They may have left the server.")
        return
    ...
    bounty["claimed_by_id"] = str(claimer.id)
    # Close all review channels for this bounty
    all_channel_ids = claim_channels.pop(bounty_id, [])

    new_total = points_data[user_id]

    # Recalculate Glory now that a new bounty is claimed, and update rank role
    rsn = verified_rsns.get(user_id)
    if rsn:
        total_level, total_xp = await fetch_osrs_stats(rsn)
        if total_level is not None and total_xp is not None:
            new_glory = calculate_glory(user_id, total_level, total_xp)
            glory_data[user_id] = {"glory": new_glory, "last_updated": datetime.now().isoformat()}
            new_rank_name = get_rank_from_glory(new_glory, user_id)
            await assign_rank_role(ctx.guild, claimer, new_rank_name)
            rank = get_rank_display(new_rank_name)
        else:
            rank = get_rank_display(get_rank_from_glory(glory_data.get(user_id, {}).get("glory", 0), user_id))
    else:
        rank = get_rank_display(get_rank_from_glory(glory_data.get(user_id, {}).get("glory", 0), user_id))
    ...
    if review_channel:
        approve_embed = discord.Embed(
            ...
            description=(
                ...
                f"**+{pts} points** and **+5 ⚔️ Glory** have been added to your total!\n"
    ...
    if bounty_channel:
        public_embed = discord.Embed(
            ...
            description=(
                ...
                f"**+{pts} points** and **+5 ⚔️ Glory** awarded!\n"
                f"**New Total:** {new_total} points\n"
                f"**Rank:** {rank}"
            ),
            color=discord.Color.green()
    ...
    save_data()
    await ctx.send(f"✅ Approved! **{claimer.display_name}** has been awarded **{pts} points** and **+5 ⚔️ Glory**.")
...
async def deny_claim(ctx, bounty_id: int = None, *, reason: str = "No reason provided."):
    ...
    review_channel = bot.get_channel(review_channel_id) if review_channel_id else None
    if review_channel:
        claimer_mention = claimer.mention if claimer else matched_claim.get("user_name", "Member")
        deny_embed = discord.Embed(
            title="❌ Claim Denied",
            description=(
                f"Sorry {claimer_mention}, your claim was denied by {ctx.author.mention}.\n\n"
                f"**Reason:** {reason}\n\n"
    ...
    # Send staff confirmation to the channel the command was used in BEFORE it gets deleted
    # (if used outside review channel, ctx.channel is safe; if inside, send now before deletion)
    save_data()
    save_data()
    deny_confirm = await ctx.send(f"❌ Claim for Bounty #{bounty_id} denied. The bounty remains active.")
    ...
    # If the confirmation was in the review channel (now deleted), send to log channel instead
    if review_channel and ctx.channel.id == review_channel_id:
        log_channel = bot.get_channel(LOG_CHANNEL_ID)
        if log_channel:
            await log_channel.send(f"❌ Bounty #{bounty_id} claim by **{matched_claim.get('user_name', 'Unknown')}** was denied by {ctx.author.mention}. Reason: {reason}")
...
async def list_bounties(ctx):
    ...
    embeds = []
    current_embed = discord.Embed(title="🎯 Active Avera Bounties", color=discord.Color.red())
    ...
    for bounty_id, b in active:
        ...
        tier = get_bounty_tier(b["points"])
        stored_tier = b.get("tier", tier["label"])
        tier_info = BOUNTY_TIERS.get(stored_tier, tier)
        field_name = f"{tier_info['emoji']} [{tier_info['label']}] #{bounty_id} — {b['target']}"
        field_value = f"💰 **{b['points']} pts + 5 ⚔️ Glory**\n📝 {b['description']}\n{status} • `!claim {bounty_id}` to claim"
        field_len = len(field_name) + len(field_value)

        # If adding this field would exceed Discord's 6000 char embed limit, start a new embed
        if current_length + field_len > 5800:
            current_embed.set_footer(text="Attach a screenshot when using !claim")
            embeds.append(current_embed)
            current_embed = discord.Embed(title="🎯 Active Avera Bounties (cont.)", color=discord.Color.red())
            current_length = len(current_embed.title)
...
async def cancel_bounty(ctx, bounty_id: int = None):
    ...
    # Clean up all open review channels concurrently
    all_channel_ids = claim_channels.pop(bounty_id, [])
    if isinstance(all_channel_ids, int):
        all_channel_ids = [all_channel_ids]  # legacy single-value safety

    async def _cancel_review_channel(ch_id):
        ch = bot.get_channel(ch_id)
        if ch:
            try:
                await ch.send("🚫 This bounty has been cancelled by an admin. Channel will be deleted in 10 seconds.")
                await asyncio.sleep(10)
                await ch.delete(reason="Bounty cancelled.")
            except (discord.NotFound, discord.Forbidden):
                pass

    if all_channel_ids:
        await asyncio.gather(*[_cancel_review_channel(ch_id) for ch_id in all_channel_ids])

    save_data()
    await ctx.send(f"✅ Bounty #{bounty_id} (**{bounties[bounty_id]['target']}**) has been cancelled.")


@bot.command(name="reopenbounty")
@commands.has_permissions(manage_guild=True)
async def reopen_bounty(ctx, bounty_id: int = None):
    """Reopen a bounty that was incorrectly closed. Admin only.
    Usage: !reopenbounty <bounty_id>"""
    if bounty_id is None or bounty_id not in bounties:
    ...
    if bounty.get("active"):
        await ctx.send(f"❌ Bounty #{bounty_id} is already active.")
        return

    # Reset the bounty to active state
    bounty["active"] = True
    bounty["claimed_by"] = None
    bounty["claimed_by_id"] = None
    bounty["pending_claims"] = []
    claim_channels.pop(bounty_id, None)
    save_data()

    reopen_tier = BOUNTY_TIERS.get(bounty.get("tier", get_bounty_tier(bounty["points"])["label"]), get_bounty_tier(bounty["points"]))
    embed = discord.Embed(
        title=f"🔄 {reopen_tier['emoji']} [{reopen_tier['label']}] Bounty #{bounty_id} Reopened",
        description=(
            f"**{bounty['target']}** is back on the board!\n\n"
            f"🏆 **Reward:** {bounty['points']} pts + 5 ⚔️ Glory\n"
            f"{reopen_tier['emoji']} **Tier:** {reopen_tier['label']}\n"
            f"📋 **Description:** {bounty.get('description', 'N/A')}\n\n"
            f"Use `!claim {bounty_id}` with a screenshot to claim it."
        ),
        color=discord.Color(reopen_tier["color"])
    )
    embed.set_footer(text=f"Reopened by {ctx.author.display_name}")
    await ctx.send(embed=embed)

    # Also post in bounty channel so members can see it's back
    bounty_channel = bot.get_channel(BOUNTY_CHANNEL_ID)
    if bounty_channel and bounty_channel.id != ctx.channel.id:
        await bounty_channel.send(embed=embed)
...
# Tracks open tickets: { user_id: { ticket_type: channel_id } }
open_tickets = {}
...
class TicketPanel(discord.ui.View):
    ...
    @discord.ui.button(label="General Support", style=discord.ButtonStyle.primary, emoji="🔧", custom_id="ticket_general", row=0)
    ...
    @discord.ui.button(label="Submissions", style=discord.ButtonStyle.secondary, emoji="📝", custom_id="ticket_submissions", row=0)
    ...
    @discord.ui.button(label="Staff Application", style=discord.ButtonStyle.success, emoji="⭐", custom_id="ticket_staffapp", row=0)
    ...
    @discord.ui.button(label="Report a Member", style=discord.ButtonStyle.danger, emoji="🚨", custom_id="ticket_report", row=1)
    ...
    @discord.ui.button(label="Rank Promotion", style=discord.ButtonStyle.success, emoji="⬆️", custom_id="ticket_promotion", row=1)
    ...
    @discord.ui.button(label="Join Clan", style=discord.ButtonStyle.primary, emoji="⚔️", custom_id="ticket_join_cc", row=1)
    async def join_clan_cc(self, interaction: discord.Interaction, button: discord.ui.Button):
        await open_join_cc_ticket(interaction)


async def open_join_cc_ticket(interaction: discord.Interaction):
    """Creates a private Join Clan ticket for new members needing to be added to the CC."""
    user = interaction.user
    guild = interaction.guild
    user_id = str(user.id)

    # Check for existing open Join Clan ticket only
    user_tickets = open_tickets.get(user_id, {})
    if "join-clan" in user_tickets:
        existing = guild.get_channel(user_tickets["join-clan"])
        if existing:
            await interaction.response.send_message(
                f"❌ You already have an open Join Clan ticket! Head to {existing.mention}.",
                ephemeral=True
            )
            return

    rsn = verified_rsns.get(user_id, "Not verified yet")
    ...
    channel_name = f"join-clan-{user.display_name}".lower().replace(" ", "-")[:100]
    ticket_channel = await guild.create_text_channel(
        name=channel_name,
        overwrites=overwrites,
        category=category,
        topic=f"Join Clan CC request for {user.display_name}"
    )

    if user_id not in open_tickets:
        open_tickets[user_id] = {}
    open_tickets[user_id]["join-clan"] = ticket_channel.id
    save_data()

    embed = discord.Embed(
        title="⚔️ Join Clan — Avera",
        description=(
            f"Hey {user.mention}! Welcome to Avera — a staff member will be with you shortly "
            f"to get you officially added to the clan CC and Wise Old Man group.\n\n"
            f"**Your RSN:** `{rsn}`\n\n"
            f"*If your RSN above is incorrect, please use `!verify <rsn>` first.*\n\n"
            f"To close this ticket, click the button below or type `!closeticket`."
        ),
        color=discord.Color.gold()
    )
    embed.set_footer(text="Avera Clan • Staff will be with you shortly")
    ...
    user_tickets = open_tickets.get(user_id, {})
    if "promotion" in user_tickets:
        existing = guild.get_channel(user_tickets["promotion"])
        if existing:
            await interaction.response.send_message(
                f"❌ You already have an open promotion ticket! Head to {existing.mention}.",
                ephemeral=True
            )
            return
    ...
    if user_id not in open_tickets:
        open_tickets[user_id] = {}
    open_tickets[user_id]["promotion"] = ticket_channel.id
    save_data()
    ...
    points = points_data.get(user_id, 0)
    months = get_months_since_verify(user_id)
    current_rank = get_rank(points, user_id, total_level, total_xp)
    rank_display = get_rank_display(current_rank)
    ...
    stats_embed.add_field(
        ...
        ) + (f"\n**Total XP:** {total_xp/1_000_000:.0f}M" if total_xp else ""),
        inline=True
    )
    ...
    stats_embed.add_field(
        name="📋 Rank System",
        value=(
            "Ranks are earned through **Glory** — a permanent score based on time in clan, "
            "level & XP milestones, bounties, referrals, and special bonuses.\n\n"
            "**Protector** → **Bulwark** → **Justiciar** → **Sentry** → "
            "**Guardian** → **Warden** → **Vanguard** → **Templar**\n\n"
            "Use `!glory` to see their full breakdown, `!rankprogress` for next rank tips."
        ),
        inline=False
    )
...
async def open_ticket(interaction: discord.Interaction, ticket_type: str, label: str, emoji: str, prompt: str):
    ...
    # Check if user already has an open ticket of this same type
    user_tickets = open_tickets.get(user_id, {})
    if ticket_type in user_tickets:
        existing = guild.get_channel(user_tickets[ticket_type])
        if existing:
            await interaction.response.send_message(
                f"❌ You already have an open {label} ticket! Head to {existing.mention}.",
                ephemeral=True
            )
            return
    ...
    if user_id not in open_tickets:
        open_tickets[user_id] = {}
    open_tickets[user_id][ticket_type] = ticket_channel.id
    save_data()
...
async def handle_close_ticket(interaction: discord.Interaction):
    ...
    # Find and remove the specific ticket type from open_tickets
    for uid, tickets in list(open_tickets.items()):
        if isinstance(tickets, dict):
            ticket_type_to_remove = next((t for t, cid in tickets.items() if cid == channel.id), None)
            if ticket_type_to_remove:
                del open_tickets[uid][ticket_type_to_remove]
                if not open_tickets[uid]:  # Clean up empty dict
                    del open_tickets[uid]
                save_data()
                break
        elif tickets == channel.id:  # Legacy single-ticket format
            del open_tickets[uid]
            save_data()
            break
...
async def close_ticket_cmd(ctx):
    ...
    ticket_type_to_remove = None

    for uid, tickets in open_tickets.items():
        if isinstance(tickets, dict):
            ticket_type = next((t for t, cid in tickets.items() if cid == ctx.channel.id), None)
            if ticket_type:
                user_id_to_remove = uid
                ticket_type_to_remove = ticket_type
                break
        elif tickets == ctx.channel.id:  # Legacy format
            user_id_to_remove = uid
            break
    ...
    if ticket_type_to_remove:
        del open_tickets[user_id_to_remove][ticket_type_to_remove]
        if not open_tickets[user_id_to_remove]:
...
async def ticket_panel(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "⚔️ **Join Clan** — New member? Click here to get added to the clan CC!\n\n"
...
flip_panel_message_id  = None  # Best Margins
flip_hv_message_id     = None  # High Volume
flip_up_message_id     = None  # Trending Up
flip_down_message_id   = None  # Trending Down
flip_guide_message_id  = None  # Guide
...
async def _refresh_dashboard(guild):
    """Silently refresh the dashboard message if it exists."""
...
def build_dashboard_embed(guild):
    ...
    if active:
        ...
        for bid, b in active[:5]:
            ...
            tier = BOUNTY_TIERS.get(b.get("tier", get_bounty_tier(b["points"])["label"]), get_bounty_tier(b["points"]))
            bounty_text += f"{tier['emoji']} {status} **#{bid} — {b['target']}** · {b['points']} pts [{tier['label']}]\n"
        if len(active) > 5:
    ...
    # Build display_name -> user_id lookup so legacy name-keyed bounties can be merged
    name_to_uid = {}
    if guild:
        for m in guild.members:
            name_to_uid[m.display_name] = str(m.id)

    bounty_counts = {}  # always keyed by user_id string (or display name if unresolvable)
    for b in bounties.values():
        if b["active"]:
            continue
        if b.get("claimed_by_id"):
            key = str(b["claimed_by_id"])
        elif b.get("claimed_by"):
            # Try to resolve legacy display name to a user ID so it merges correctly
            key = name_to_uid.get(b["claimed_by"], b["claimed_by"])
        else:
            continue
        bounty_counts[key] = bounty_counts.get(key, 0) + 1

    if bounty_counts:
        top_hunters = sorted(bounty_counts.items(), key=lambda x: x[1], reverse=True)[:3]
        medals = ["🥇", "🥈", "🥉"]
        hunters_text = ""
        for i, (key, count) in enumerate(top_hunters):
            if key.isdigit():
                member = guild.get_member(int(key)) if guild else None
                display = member.display_name if member else verified_rsns.get(key, f"Unknown ({key})")
            else:
                display = key  # Unresolvable legacy name
            hunters_text += f"{medals[i]} **{display}** — {count} {'bounties' if count != 1 else 'bounty'}\n"
    else:
...
async def coffer_deposit(ctx, member: discord.Member = None, amount: int = None, *, note: str = ""):
    ...
    await _refresh_dashboard(ctx.guild)
...
class RedeemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Re-validate balance (guards against double-submits or race conditions)
        current_balance = points_data.get(user_id, 0)
        if current_balance < cost:
            await interaction.response.send_message(
                f"❌ You don't have enough points to redeem this! "
                f"**{self.item['name']}** costs **{cost} pts** but you only have **{current_balance} pts**.",
                ephemeral=True
            )
            return

        # Deduct points
        points_data[user_id] = current_balance - cost
        ...
        redemption_index = len(shop_redemptions)  # 1-based index matching !fulfil and !redemptions
...
async def shop_panel(ctx):
    """Post a permanent shop panel. Admin only. Usage: !shoppanel"""
    embed = build_shop_embed(ctx.guild)
    await ctx.send(embed=embed, view=ShopView())
    if ctx.guild:
        await ctx.message.delete()
...
async def welcome_panel(ctx):
    ...
    embed.add_field(
        name="⚔️ Rank System",
        value=(
            "Avera has a unique rank system built around **Glory** — a permanent score earned through your time in the clan, "
            "in-game progression, bounties and more.\n\n"
            "Ranks go from **Protector** all the way up to **Templar** — use `!glory` to see your score "
            "and `!rankprogress` to see exactly what you need for your next rank.\n"
            f"📌 Check the FAQ for the full breakdown!"
        ),
        inline=False
    )
...
async def bounty_panel(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "📚 **Skill of the Week** — 500 pts — Choose the next skill the clan grinds\n"
            "💀 **Boss of the Week** — 500 pts — Choose the next featured boss\n"
            "🏠 **Create Your Own Room** — 1,000 pts — Get your own custom Discord category & voice channel\n\n"
...
weekly_stats = {
    ...
    "last_recap_date": None  # ISO date string of last recap, prevents duplicate sends on restart
}
    ...
    for guild in bot.guilds:
        ...
        for milestone in MEMBER_MILESTONES:
            if count >= milestone and milestone not in announced_milestones:
                ...
                save_data()  # Persist so milestone doesn't re-fire on restart
...
async def weekly_recap():
    ...
    # Skip if we already sent a recap today (prevents duplicate on bot restart)
    today_str = now.strftime("%Y-%m-%d")
    if weekly_stats.get("last_recap_date") == today_str:
        return

    # Save the date FIRST before sending — prevents duplicate if bot restarts mid-task
    weekly_stats["last_recap_date"] = today_str
    save_data()
...
async def create_poll(ctx, duration: int = None, *, details: str = None):
    ...
    active_polls[msg.id] = {
        ...
        "duration": duration,
        "ends_at": (datetime.now().timestamp() + duration * 60)
    }
    save_data()
...
@bot.event
async def on_ready_resume_polls():
    pass  # Handled inline in on_ready below via resume_active_polls()


async def resume_active_giveaways():
    """On startup, resume any giveaways that were still running before the bot restarted."""
    if not active_giveaways_live:
        return
    for msg_id, g in list(active_giveaways_live.items()):
        channel = bot.get_channel(g["channel_id"])
        if not channel:
            active_giveaways_live.pop(msg_id, None)
            continue
        remaining = g["end_time"] - datetime.now().timestamp()
        if remaining <= 0:
            remaining = 0  # Already expired — pick winner immediately
        bot.loop.create_task(_run_giveaway(msg_id, g["prize"], g.get("host_name", "Unknown"), channel, remaining))


async def _run_giveaway(msg_id, prize, host_name, channel, remaining_seconds):
    """Wait out the remaining time then pick a giveaway winner. Used by both !giveaway and resume."""
...
async def resume_active_polls():
    """On startup, resume any polls that were running before the bot restarted."""
    if not active_polls:
        return
    number_emojis = ["1️⃣","2️⃣","3️⃣","4️⃣","5️⃣"]
    for msg_id, poll in list(active_polls.items()):
        ...
        if not channel:
            ...
            continue
        ends_at = poll.get("ends_at", 0)
        remaining = ends_at - datetime.now().timestamp()
        if remaining <= 0:
            remaining = 0  # Already expired — close immediately

        async def _close_poll(mid=msg_id, ch=channel, p=poll, r=remaining):
            await asyncio.sleep(r)
            try:
                msg = await ch.fetch_message(mid)
            except discord.NotFound:
                active_polls.pop(mid, None)
                return
            results = []
            ne = ["1️⃣","2️⃣","3️⃣","4️⃣","5️⃣"]
            for i, option in enumerate(p["options"]):
                reaction = discord.utils.get(msg.reactions, emoji=ne[i])
                count = (reaction.count - 1) if reaction else 0
                results.append((option, count))
            results.sort(key=lambda x: x[1], reverse=True)
            winner = results[0]
            result_text = "\n".join([
                f"{ne[i]} **{opt}** — {count} vote(s) {'👑' if i == 0 else ''}"
                for i, (opt, count) in enumerate(results)
            ])
            result_embed = discord.Embed(
                title=f"📊 Poll Closed — {p['question']}",
                description=(
                    f"{result_text}\n\n"
                    f"🏆 **Winner: {winner[0]}** with {winner[1]} vote(s)!"
                    if winner[1] > 0 else f"{result_text}\n\n*No votes were cast.*"
            ...
            result_embed.set_footer(text="Poll resumed after bot restart")
            result_embed.timestamp = datetime.now()
            await ch.send(embed=result_embed)
            active_polls.pop(mid, None)
            save_data()

        bot.loop.create_task(_close_poll())


# ── Streak Rewards ────────────────────────────────────────────

def update_streak(user_id, display_name):
    """Call when a member attends an event. Returns (streak, bonus_points) — caller awards the points."""
    event_streaks[user_id] = event_streaks.get(user_id, 0) + 1
    streak = event_streaks[user_id]
    if streak % STREAK_THRESHOLD == 0:
        return streak, STREAK_BONUS  # Caller is responsible for awarding the bonus
    return streak, 0
...
async def verify_rsn(ctx, *, rsn: str = None):
    ...
    # Auto-assign rank role on verify using Glory
    total_level, total_xp = await fetch_osrs_stats(rsn)
    if total_level is not None and total_xp is not None:
        glory = calculate_glory(user_id, total_level, total_xp)
        glory_data[user_id] = {"glory": glory, "last_updated": datetime.now().isoformat()}
    else:
        # Hiscores failed — use existing cached glory if available, otherwise default to 0
        glory = glory_data.get(user_id, {}).get("glory", 0)
    new_rank = get_rank_from_glory(glory, user_id)
    await assign_rank_role(ctx.guild, ctx.author, new_rank)

    # WOM check — skip for staff/admin-ranked members and re-verifies
    # old_rsn is None means this is their first ever verify (verify_times was just set above)
    is_first_verify = old_rsn is None
    if user_id not in member_ranks and is_first_verify:
        in_wom = await is_player_in_wom_group(rsn)
        if in_wom is False:  # Explicitly not in group (None means API error, don't penalise)
            wom_pending[user_id] = datetime.now().isoformat()
            save_data()
            try:
                wom_dm = discord.Embed(
                    title="⚔️ Welcome to Avera!",
                    description=(
                        f"Hey **{ctx.author.display_name}**! 👋 Welcome to Avera — we're so glad you're here!\n\n"
                        f"To complete your membership, please reach out to a staff member so we can get you officially added to the clan CC. "
                        f"This is the last step to becoming a full member!\n\n"
                        f"📌 Head to <#{TICKET_PANEL_CHANNEL_ID}> and click the **⚔️ Join Clan** button to open a ticket and get sorted!\n\n"
                        f"⏰ Just a heads up — if we haven't heard from you within **{WOM_NOTIFY_HOURS} hours**, our system will automatically flag your account for staff review. "
                        f"Please create a ticket if you need more time and we will discuss next steps to make sure you are taken care of.\n\n"
                        f"If you have any questions at all, don't hesitate to ask in the server. We look forward to seeing you around! ⚔️🍀"
                    ),
                    color=discord.Color.gold()
                )
                await ctx.author.send(embed=wom_dm)
            except discord.Forbidden:
                pass
...
@bot.command(name="addglory")
@commands.has_permissions(manage_guild=True)
async def add_glory(ctx, member: discord.Member = None, amount: float = None, *, reason: str = "No reason given"):
    """Add a permanent Glory bonus to a member. Admin only.
    Usage: !addglory @member <amount> [reason]
    Example: !addglory @Jerzi 50 Won the skilling event"""
    if not member or amount is None:
        await ctx.send("❌ Usage: `!addglory @member <amount> [reason]`")
        return
    if amount <= 0:
        await ctx.send("❌ Amount must be a positive number.")
        return
    ...
    user_id = str(member.id)
    glory_bonus[user_id] = glory_bonus.get(user_id, 0) + amount
    save_data()

    # Recalculate glory and update rank role
    rsn = verified_rsns.get(user_id)
    if rsn:
        total_level, total_xp = await fetch_osrs_stats(rsn)
        if total_level is not None and total_xp is not None:
            new_glory = calculate_glory(user_id, total_level, total_xp)
            glory_data[user_id] = {"glory": new_glory, "last_updated": datetime.now().isoformat()}
            new_rank = get_rank_from_glory(new_glory, user_id)
            await assign_rank_role(ctx.guild, member, new_rank)
            save_data()

    embed = discord.Embed(
        title="⚔️ Glory Bonus Awarded",
        description=(
            f"{member.mention} has been awarded **+{amount:.0f} Glory**!\n\n"
            f"📝 **Reason:** {reason}\n"
            f"🎁 **Total bonus Glory:** {glory_bonus[user_id]:.0f}\n\n"
            f"*Their rank has been updated automatically.*"
        ),
        color=discord.Color.gold()
    )
    embed.set_footer(text=f"Awarded by {ctx.author.display_name}")
...
@bot.command(name="removeglory")
@commands.has_permissions(manage_guild=True)
async def remove_glory(ctx, member: discord.Member = None, amount: float = None, *, reason: str = "No reason given"):
    """Remove Glory bonus from a member. Admin only.
    Usage: !removeglory @member <amount> [reason]"""
    if not member or amount is None:
        await ctx.send("❌ Usage: `!removeglory @member <amount> [reason]`")
        return
    if amount <= 0:
        await ctx.send("❌ Amount must be a positive number.")
        return
    ...
    user_id = str(member.id)
    current_bonus = glory_bonus.get(user_id, 0)
    glory_bonus[user_id] = max(0, current_bonus - amount)
    save_data()
    ...
    embed = discord.Embed(
        title="⚔️ Glory Bonus Removed",
        description=(
            f"Removed **{amount:.0f} Glory** from {member.mention}.\n\n"
            f"📝 **Reason:** {reason}\n"
            f"🎁 **Remaining bonus Glory:** {glory_bonus[user_id]:.0f}\n\n"
            f"*Their rank has been updated automatically.*"
        ),
        color=discord.Color.red()
    )
...
@bot.command(name="setverifydate")
@commands.has_permissions(manage_guild=True)
async def set_verify_date(ctx, member: discord.Member = None, *, date: str = None):
    """Manually set a member's verify date (for backdating). Admin only.
    Usage: !setverifydate @member YYYY-MM-DD
    Example: !setverifydate @Jerzi 2024-06-01"""
    if not member or not date:
        await ctx.send("❌ Usage: `!setverifydate @member YYYY-MM-DD`\nExample: `!setverifydate @Jerzi 2024-06-01`")
        return

    try:
        parsed = datetime.strptime(date.strip(), "%Y-%m-%d")
    except ValueError:
        await ctx.send("❌ Invalid date format. Use `YYYY-MM-DD` e.g. `2024-06-01`")
        return

    user_id = str(member.id)
    if user_id not in verified_rsns:
        await ctx.send(f"❌ **{member.display_name}** hasn't verified an RSN yet.")
        return

    verify_times[user_id] = parsed.isoformat()
    save_data()

    months = get_months_since_verify(user_id)
    await ctx.send(
        f"✅ Set verify date for **{member.display_name}** to `{date}` "
        f"({months:.1f} months ago). Run `!glory` to recalculate their Glory."
    )
...
async def hiscores_debug(ctx, *, rsn: str = None):
    """Debug raw hiscores response. Admin only. Usage: !hiscoresdebug <rsn>"""
    if not rsn:
        rsn = verified_rsns.get(str(ctx.author.id))
        if not rsn:
...
@bot.command(name="womcheck")
@commands.has_permissions(manage_guild=True)
async def wom_check(ctx):
    """Check which verified members are missing from the WOM group. Admin only."""
    to_check = {uid: rsn for uid, rsn in verified_rsns.items() if uid not in member_ranks}
    if not to_check:
        await ctx.send("ℹ️ No verified non-staff members to check.")
        return

    msg = await ctx.send(f"🔍 Checking **{len(to_check)}** members against WOM... (0/{len(to_check)})")
    missing = []
    errors = []

    for i, (user_id, rsn) in enumerate(to_check.items()):
        result = await is_player_in_wom_group(rsn)
        if result is False:
            member = ctx.guild.get_member(int(user_id))
            name = member.display_name if member else f"Unknown ({user_id})"
            missing.append((name, rsn))
        elif result is None:
            errors.append(rsn)
        # Update progress every 5 members
        if (i + 1) % 5 == 0:
            await msg.edit(content=f"🔍 Checking **{len(to_check)}** members against WOM... ({i+1}/{len(to_check)})")
        await asyncio.sleep(0.5)  # Be polite to WOM API

    if not missing and not errors:
        await msg.edit(content="✅ All verified members are in the WOM group!")
        return

    embed = discord.Embed(
        title="⚠️ Members Missing from WOM Group",
        description=f"🔗 https://wiseoldman.net/groups/{WOM_GROUP_ID}",
        color=discord.Color.orange()
    )
    if missing:
        lines = "\n".join(f"• **{name}** — `{rsn}`" for name, rsn in missing)
        embed.add_field(name=f"❌ Not in WOM ({len(missing)})", value=lines[:1024], inline=False)
    if errors:
        embed.add_field(name=f"⚠️ Could not check ({len(errors)})", value="\n".join(f"• `{r}`" for r in errors[:20]), inline=False)
    embed.set_footer(text="Use !womclear @member to dismiss an alert after adding them to WOM")
    await msg.edit(content=None, embed=embed)


@tasks.loop(hours=1)
async def wom_pending_check():
    """Hourly check — if a pending member still isn't in WOM after 24hrs, alert staff."""
    global _wom_check_running
    if _wom_check_running:
        return  # Previous run still in progress — skip this cycle
    _wom_check_running = True
    try:
        if not wom_pending:
            return
        ...
        for user_id, notified_at in list(wom_pending.items()):
            rsn = verified_rsns.get(user_id)
            if not rsn:
                to_clear.append(user_id)
                continue

            # If they're now in WOM, clear them silently
            in_wom = await is_player_in_wom_group(rsn)
            await asyncio.sleep(0.5)  # Be polite to WOM API
            if in_wom is True:
                to_clear.append(user_id)
                continue
            if in_wom is None:
                continue  # API error — skip this cycle, don't penalise

            # Check if 24 hours have passed since notification
            # Handle both old string format and new dict format
            if isinstance(notified_at, dict):
                notified_dt = datetime.fromisoformat(notified_at.get("notified_at", datetime.now().isoformat()))
            else:
                notified_dt = datetime.fromisoformat(notified_at)
            hours_elapsed = (datetime.now() - notified_dt).total_seconds() / 3600

            # Alert every 24 hours until resolved — store last_alerted in the value
            # wom_pending value is either an iso string OR a dict with notified_at + last_alerted
            if isinstance(wom_pending[user_id], dict):
                last_alerted_str = wom_pending[user_id].get("last_alerted")
                last_alerted_dt = datetime.fromisoformat(last_alerted_str) if last_alerted_str else notified_dt
            else:
                last_alerted_dt = notified_dt

            hours_since_last_alert = (datetime.now() - last_alerted_dt).total_seconds() / 3600

            if hours_elapsed >= WOM_NOTIFY_HOURS and hours_since_last_alert >= WOM_NOTIFY_HOURS:
                if log_channel:
                    ...
                    days_waiting = int(hours_elapsed / 24)
                    time_str = f"{days_waiting} day{'s' if days_waiting != 1 else ''}" if days_waiting >= 1 else f"{int(hours_elapsed)} hours"

                    embed = discord.Embed(
                        title="🚨 WOM Review Required",
                        description=(
                            f"{mention} verified as **{rsn}** but has still not been added to the WOM group "
                            f"after **{time_str}**.\n\n"
                            f"🔗 https://wiseoldman.net/groups/{WOM_GROUP_ID}\n\n"
                            f"Please review and either:\n"
                            f"• Add them to WOM — alert will clear automatically\n"
                            f"• Use `!womclear @member` to manually dismiss\n"
                            f"• Kick them from the server if they shouldn't be here"
                        ),
                        color=discord.Color.red()
                    )
                    embed.set_footer(text=f"First notified: {notified_dt.strftime('%Y-%m-%d %H:%M')} UTC • This alert repeats every 24hrs until resolved")
                    await log_channel.send(embed=embed)

                # Update last_alerted timestamp, keep in pending
                wom_pending[user_id] = {
                    "notified_at": notified_at if isinstance(notified_at, str) else notified_at.get("notified_at"),
                    "last_alerted": datetime.now().isoformat()
                }
                save_data()

        for user_id in to_clear:
            wom_pending.pop(user_id, None)
    ...
    @wom_pending_check.before_loop
    async def before_wom_pending_check():
        await bot.wait_until_ready()


    @bot.command(name="womclear")
    @commands.has_permissions(manage_guild=True)
    async def wom_clear(ctx, member: discord.Member = None):
        """Dismiss a WOM pending alert for a member. Admin only.
        Usage: !womclear @member"""
        if not member:
            await ctx.send("❌ Usage: `!womclear @member`")
            return
        user_id = str(member.id)
        if user_id in wom_pending:
            del wom_pending[user_id]
            save_data()
            await ctx.send(f"✅ WOM alert cleared for **{member.display_name}**.")
        else:
            await ctx.send(f"ℹ️ No pending WOM alert for **{member.display_name}**.")
    ...
    async def faq_panel(ctx):
        ...
        BANNER_URL = "https://cdn.discordapp.com/attachments/1479995628825215117/1479997005374947339/image.png?ex=69b00c5f&is=69aebadf&hm=c08c648d5c904f7f664b867dfaaff7a6da26f3d36b3c4631e25a630b9615063f&"
        ...
        embed1 = discord.Embed(
            description=(
                "## ⚔️ Welcome to Avera\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "Avera is an Old School RuneScape clan built on friendship, fun, and gains. "
                "Whether you're a seasoned veteran or just starting out, there's a place for you here.\n\n"
                "The bot powers our entire clan experience — ranks, bounties, events, points and more. "
                "Use `!help` at any time for a full command list.\n\n"
                "## 👤 Getting Started\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "> `!verify <rsn>` — Link your OSRS username to Discord\n"
                "> `!whoami` — Check which RSN is linked to you\n"
                "> `!referral @member` — Credit whoever recruited you *(+10 pts to them)*\n"
                "> `!points` — Check your clan point balance\n"
                "> `!profile` — View your full clan profile\n"
        ...
        embed2 = discord.Embed(
            description=(
                "## 🏅 Clan Points & Shop\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "Clan points are your currency — earn them through activity and spend them in the shop.\n\n"
                "**💰 How to earn points:**\n"
                "> 🎯 Complete a bounty — *points vary per bounty*\n"
                "> 📅 Attend a clan event with `!attend` — **+25 pts**\n"
                "> 🔥 Attend 3 events in a row — **+5 bonus pts** streak\n"
                "> 🌟 Host a clan event — *variable, set by admin*\n"
                "> 🥇 Finish 1st in a competition — **+50 pts**\n"
                "> 🥈 Finish 2nd in a competition — **+25 pts**\n"
                "> 🥉 Finish 3rd in a competition — **+10 pts**\n"
                "> 🌱 Successfully refer a new member — **+10 pts**\n\n"
                "**🛒 Shop Commands:**\n"
                "> `!shop` — Browse the clan shop\n"
                "> `!redeem <#>` — Spend your points on a reward\n"
                f"> 📌 View the full shop in <#1479472183221682298>"
            ),
            color=discord.Color.gold()
        )
        embed2.set_footer(text="Avera Clan • Points never expire")

        # ── Embed 3: Bounties & Events ──────────────────────────
        embed3 = discord.Embed(
            description=(
                "## 🎯 Bounties\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "Bounties are clan challenges — complete them for points and Glory. "
                "Always attach a **timestamped screenshot** as proof when claiming!\n\n"
                "> `!bounties` — View all active bounties\n"
                "> `!claim <id>` — Submit your claim with proof\n"
                f"> 📌 Check <#{BOUNTY_CHANNEL_ID}> for active bounties\n\n"
                "## 📅 Events\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "Clan events are held regularly — showing up earns you points and builds your streak!\n\n"
                "> `!events` — View upcoming clan events\n"
                "> `!attend` — Check in to an active event **(+25 pts)**\n"
                "> `!streak` — Check your consecutive attendance streak\n"
                "> 🔥 Hit a **3 event streak** for a **+5 bonus pts** reward"
            ),
            color=discord.Color.gold()
        )
        embed3.set_footer(text="Avera Clan • Stay active, stay rewarded")

        # ── Embed 4: Rank System ────────────────────────────────
        rank_lines = ""
        for rank_name, threshold in reversed(GLORY_THRESHOLDS):
            emoji = RANK_EMOJIS.get(rank_name, "⚔️")
            rank_lines += f"> {emoji} **{rank_name}** — {threshold} Glory\n"

        embed4 = discord.Embed(
            description=(
                "## ⚔️ The Glory Rank System\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "Ranks in Avera are earned through **Glory** — a permanent score that reflects "
                "everything you've contributed to the clan. Glory **never decreases**, so every "
                "bit of effort counts forever.\n\n"
                "**🎖️ Ranks — Lowest to Highest:**\n"
                f"{rank_lines}\n"
                "> 🛡️ **Moderator** — Staff assigned only, sits above all earned ranks\n\n"
                "**✨ How Glory is earned:**\n"
                "> ⏱️ **Time in clan** — 15 Glory per month\n"
                "> 📊 **Total Level milestones** — 500, 750, 1000, 1250, 1500, 1750, 2000, 2200, 2376\n"
                "> 🧮 **Total XP milestones** — 50M, 100M, 200M, 500M, 1B, 2B XP\n"
                "> 🎯 **Bounties claimed** — +5 Glory each *(biggest driver!)*\n"
                "> 👥 **Referrals** — +10 Glory each\n"
                "> 🎁 **Staff bonuses** — awarded for special contributions\n\n"
                "**📈 Check your progress:**\n"
                "> `!glory` — Full Glory score and breakdown\n"
                "> `!rankprogress` — Tips on what to do next\n"
                "> `!rankcheck` — Refresh your rank role"
            ),
            color=discord.Color.gold()
        )
        embed4.set_footer(text="Avera Clan • Glory is permanent — every contribution counts")

        # ── Embed 5: Useful Tools ───────────────────────────────
        embed5 = discord.Embed(
            description=(
                "## 🌐 Helpful OSRS Resources\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "A collection of the most useful tools for any OSRS player.\n\n"
                "> 📖 **OSRS Wiki** — Everything in the game\n"
                "> 🔗 https://oldschool.runescape.wiki\n\n"
                "> 🏆 **OSRS Hiscores** — Look up any player's stats\n"
                "> 🔗 https://secure.runescape.com/m=hiscore_oldschool/overall\n\n"
                "> 📈 **GE Tracker** — Prices, flipping margins, history\n"
                "> 🔗 https://www.ge-tracker.com\n\n"
                "> 👥 **Wise Old Man** — Track your gains and group progress\n"
                "> 🔗 https://wiseoldman.net\n\n"
                "> ⚔️ **DPS Calculator** — Max hit, DPS, gear setups\n"
                "> 🔗 https://dps.osrs.wiki\n\n"
        ...
        embed5.set_footer(text="Avera Clan • Happy scaping! 🍀")

        # ── Embed 6: Other Features ─────────────────────────────
        embed6 = discord.Embed(
            description=(
                "## 🔍 Other Commands & Features\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
                "**Player Lookup:**\n"
                "> `!stats <rsn>` — View any player's OSRS stats\n"
                "> `!kc <rsn>` — View any player's boss kill counts\n"
                "> `!whois @member` — See a member's verified RSN\n"
                "> `!compare @member1 @member2` — Compare two members side by side\n\n"
                "**Clan Coffer:**\n"
                "> `!cofferbalance` — Check the clan coffer balance\n"
                f"> 📌 Donations and payouts tracked live in <#1479551190189736199>\n\n"
                "**Polls:**\n"
                "> Keep an eye on <#1459412856146624643> for staff polls\n"
                "> React with the number emoji to vote — results post when the poll closes\n\n"
                "**Referrals:**\n"
                "> Did someone bring you to Avera? Give them credit!\n"
                "> Use `!referral @member` after verifying to credit your recruiter\n"
                "> They receive **+10 pts** and **+10 Glory** as a thank you\n"
                "> *Each member can only submit one referral — make it count!*"
            ),
            color=discord.Color.gold()
        )
        embed6.set_footer(text="Avera Clan • Together we grind 🍀")
    ...
    async def verify_panel(ctx):
        ...
        embed.add_field(
            name="🌍 Once Verified",
            value=(
                "You\'ll gain access to the full server! Here\'s what to do next:\n\n"
                f"**1.** Run `!rankcheck` — assigns your Discord rank based on your in-game stats\n"
                f"**2.** Check the FAQ in <#{FAQ_CHANNEL_ID}>\n"
                f"**3.** Grab your roles in <#{REACTION_ROLES_CHANNEL_ID}>\n"
                f"**4.** Browse bounties in <#{BOUNTY_CHANNEL_ID}>\n"
                f"**5.** Join us in-game — search **\"Avera\"** in the clan CC!\n\n"
...
async def start_giveaway(ctx, duration: str = None, *, prize: str = None):
    ...
    # Run giveaway via shared helper (also used by resume on restart)
    bot.loop.create_task(_run_giveaway(msg.id, prize, ctx.author.display_name, channel, seconds))
...
FLIP_CHANNEL_ID = 1480848075164614718
LOG_CHANNEL_ID = 1480374547558567977
MEMBER_JOIN_CHANNEL_ID = 1479472357201416382
WOM_GROUP_ID = 24101
WOM_NOTIFY_HOURS = 24  # Hours before staff report is generated
...
# Personal flip tracker: { user_id: [ { item, buy_price, sell_price, quantity, profit, timestamp } ] }
GE_TAX_RATE = 0.01        # 1% GE tax (capped at 5m per trade)
...
WIKI_5M_URL      = "https://prices.runescape.wiki/api/v1/osrs/5m"
WIKI_6H_URL      = "https://prices.runescape.wiki/api/v1/osrs/6h"
WIKI_24H_URL     = "https://prices.runescape.wiki/api/v1/osrs/24h"
WIKI_HEADERS     = {"User-Agent": "Avera Clan Discord Bot — contact via Discord"}

TOP_MARGINS_COUNT  = 10
MIN_VOLUME         = 10        # Minimum 6h volume for Best Margins
MIN_MARGIN         = 5_000     # Minimum margin (gp) for Best Margins
HV_MIN_VOLUME      = 2_500     # Minimum 6h volume for High Volume category
HV_MIN_MARGIN      = 100       # Minimum margin (gp) for High Volume category
TREND_MIN_VOLUME   = 100       # Minimum 6h volume to appear in Trending
TREND_MIN_CHANGE   = 2.0       # Minimum % price change to appear in Trending
TREND_MIN_PRICE    = 1_000     # Minimum item price to filter out junk/noise


def format_gp(amount) -> str:
    """Format a GP value into a readable string e.g. 1234567 -> 1.23m, -2000000 -> -2.00m"""
    try:
        amount = int(amount)
...
def calc_profit(buy: int, sell: int, quantity: int) -> int:
    """Calculate net profit after GE tax."""
    tax_per_item = min(int(sell * GE_TAX_RATE), GE_TAX_CAP)
    return (sell - buy - tax_per_item) * quantity


def get_best_prices(item_id: str, prices_5m, prices_raw):
    """Return (avg_high, avg_low) for an item.
    Prefers 5m rolling averages for accuracy; falls back to /latest snapshot.
    Also returns a bool indicating whether 5m data was used."""
    if prices_5m:
        d5 = prices_5m.get(item_id, {})
        h5 = d5.get("avgHighPrice")
        l5 = d5.get("avgLowPrice")
        if h5 and l5:
            return h5, l5, True
    # Fall back to latest snapshot
    dl = prices_raw.get(item_id, {}) if prices_raw else {}
    return dl.get("high"), dl.get("low"), False


def calc_margin(high: int, low: int) -> int:
    """Instant-sell margin after tax on the high price."""
    tax = min(int(high * GE_TAX_RATE), GE_TAX_CAP)
    return high - low - tax


async def _cache_ge_mapping():
    """Fetch and cache the GE item name mapping once on startup."""
    global ge_item_name_map
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            async with session.get(WIKI_MAPPING_URL, timeout=aiohttp.ClientTimeout(total=15)) as r:
                mapping_raw = await r.json()
        ge_item_name_map  = {str(item["id"]): item["name"]              for item in mapping_raw}
        ge_item_limit_map = {str(item["id"]): item.get("limit", None)   for item in mapping_raw}
        print(f"✅ GE item mapping cached — {len(ge_item_name_map)} items")
    except Exception as e:
        print(f"⚠️ Failed to cache GE mapping: {e}")


async def fetch_ge_prices():
    """Fetch GE prices from OSRS Wiki. Returns (prices_raw, volumes_raw, prices_5m, prices_24h, name_map).
    - prices_raw:  latest single-transaction snapshot (/latest) — kept for last-trade reference only
    - volumes_raw: 6h trade counts per item
    - prices_5m:   5-minute rolling averages — used for margin calc and suggested offer prices
    - prices_24h:  24h averages — used for trending
    Each secondary fetch is isolated so a failure only affects its own category."""
    prices_raw  = None
    volumes_raw = None
    prices_5m   = None
    prices_24h  = None
    ...
    # Core fetch — latest snapshot + 6h volume (both required)
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            async with session.get(WIKI_PRICES_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                prices_raw = (await r.json()).get("data", {})
            async with session.get(WIKI_6H_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                raw_6h = (await r.json()).get("data", {})
                volumes_raw = {
                    item_id: (d.get("highPriceVolume", 0) or 0) + (d.get("lowPriceVolume", 0) or 0)
                    for item_id, d in raw_6h.items()
                }
    except Exception as e:
        print(f"⚠️ GE prices/volumes fetch failed: {e}")
        return None, None, None, None, None

    # 5m fetch — isolated; used for suggested offers and margin calculations
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            async with session.get(WIKI_5M_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                prices_5m = (await r.json()).get("data", {})
    except Exception as e:
        print(f"⚠️ GE 5m fetch failed (falling back to /latest for margins): {e}")
        prices_5m = None

    # 24h fetch — isolated; used for trending categories only
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            async with session.get(WIKI_24H_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                prices_24h = (await r.json()).get("data", {})
    except Exception as e:
        print(f"⚠️ GE 24h fetch failed (trending unavailable): {e}")
        prices_24h = None

    # Name map — use cache if available, else fetch inline
    try:
        if ge_item_name_map:
            name_map = ge_item_name_map
        else:
            async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
                async with session.get(WIKI_MAPPING_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    mapping_raw = await r.json()
            name_map = {str(item["id"]): item["name"] for item in mapping_raw}
            ge_item_limit_map.update({str(item["id"]): item.get("limit", None) for item in mapping_raw})
    except Exception as e:
        print(f"⚠️ GE mapping fetch failed: {e}")
        return None, None, None, None, None

    return prices_raw, volumes_raw, prices_5m, prices_24h, name_map


async def fetch_item_price(item_name: str):
    """Look up a specific item by name. Returns (item_id, name, high, low, margin) or None."""
    prices_raw, volumes_raw, prices_5m, prices_24h, name_map = await fetch_ge_prices()
    if not prices_raw:
        return None
    def build_result(item_id, name):
        high_5m, low_5m, used_5m = get_best_prices(item_id, prices_5m, prices_raw)
        # Last-trade snapshot for reference
        snap = prices_raw.get(item_id, {}) if prices_raw else {}
        last_high = snap.get("high")
        last_low  = snap.get("low")
        if not high_5m or not low_5m:
            return {"id": item_id, "name": name, "high": None, "low": None, "margin": None, "volume": 0}
        margin        = calc_margin(high_5m, low_5m)
        suggest_buy   = low_5m  + 1   # 1gp above 5m avg — fills faster than most buyers
        suggest_sell  = high_5m - 1   # 1gp below 5m avg — undercuts most sellers
        vol           = volumes_raw.get(item_id, 0) if volumes_raw else 0
        return {
            "id":           item_id,
            "name":         name,
            "high":         high_5m,      # 5m avg high (used for margin)
            "low":          low_5m,        # 5m avg low  (used for margin)
            "suggest_buy":  suggest_buy,
            "suggest_sell": suggest_sell,
            "last_high":    last_high,     # raw /latest snapshot
            "last_low":     last_low,
            "margin":       margin,
            "volume":       vol,
            "used_5m":      used_5m,
    ...
    for item_id, name in name_map.items():
        if name.lower() == query:
            exact_match_name = name
            return build_result(item_id, name)
    if not exact_match_name:
        matches = [(iid, n) for iid, n in name_map.items() if query in n.lower()]
        if matches:
...
# ══════════════════════════════════════════════════════════════════════════════
#  GEAR COMMAND  —  !gear <boss>
#  Fetches OSRS Wiki strategy page, parses available combat styles,
#  lets the user pick via buttons, then uses Claude AI to format a full
#  gear/inventory/prayer embed per style. Cached per boss per style for 24h.
# ══════════════════════════════════════════════════════════════════════════════

WIKI_API_URL    = "https://oldschool.runescape.wiki/api.php"
ANTHROPIC_URL   = "https://api.anthropic.com/v1/messages"
GEAR_CACHE_TTL  = 86400  # 24 hours in seconds

# Common boss name aliases → wiki page name
BOSS_ALIASES = {
    "zulrah": "Zulrah", "zul": "Zulrah",
    "vorkath": "Vorkath", "vork": "Vorkath",
    "corp": "Corporeal Beast", "corporeal": "Corporeal Beast", "corporeal beast": "Corporeal Beast",
    "cox": "Chambers of Xeric", "chambers": "Chambers of Xeric", "raids1": "Chambers of Xeric",
    "tob": "Theatre of Blood", "theatre": "Theatre of Blood", "raids2": "Theatre of Blood",
    "toa": "Tombs of Amascut", "tombs": "Tombs of Amascut", "raids3": "Tombs of Amascut",
    "gwd": "God Wars Dungeon",
    "kree": "Kree'arra", "kreearra": "Kree'arra",
    "zilyana": "Commander Zilyana", "sara": "Commander Zilyana", "zily": "Commander Zilyana",
    "graardor": "General Graardor", "bandos": "General Graardor",
    "zammy": "K'ril Tsutsaroth", "kril": "K'ril Tsutsaroth", "zamorak": "K'ril Tsutsaroth",
    "cerb": "Cerberus", "cerberus": "Cerberus",
    "hydra": "Alchemical Hydra", "alch hydra": "Alchemical Hydra",
    "kraken": "Kraken",
    "thermy": "Thermonuclear Smoke Devil", "thermonuclear": "Thermonuclear Smoke Devil",
    "dag kings": "Dagannoth Kings", "dagannoth kings": "Dagannoth Kings", "dks": "Dagannoth Kings",
    "muspah": "Phantom Muspah", "phantom muspah": "Phantom Muspah",
    "duke": "Duke Sucellus",
    "leviathan": "The Leviathan", "levi": "The Leviathan",
    "whisperer": "The Whisperer", "whisp": "The Whisperer",
    "vardorvis": "Vardorvis", "vard": "Vardorvis",
    "nex": "Nex",
    "nightmare": "The Nightmare", "nm": "The Nightmare",
    "phosani": "Phosani's Nightmare", "phosanis": "Phosani's Nightmare",
    "gauntlet": "The Gauntlet", "corrupted gauntlet": "Corrupted Gauntlet", "cg": "Corrupted Gauntlet",
    "grotesque": "Grotesque Guardians", "gg": "Grotesque Guardians",
    "abyssal sire": "Abyssal Sire", "sire": "Abyssal Sire",
    "demonic gorilla": "Demonic gorilla", "demonics": "Demonic gorilla", "gorillas": "Demonic gorilla",
    "kalphite queen": "Kalphite Queen", "kq": "Kalphite Queen",
    "king black dragon": "King Black Dragon", "kbd": "King Black Dragon",
    "sarachnis": "Sarachnis",
    "scorpia": "Scorpia",
    "callisto": "Callisto",
    "venenatis": "Venenatis",
    "vetion": "Vet'ion", "vet'ion": "Vet'ion",
...
async def fetch_wiki_wikitext(boss_name: str) -> str | None:
    """Fetch raw wikitext for a boss's Strategies page from the OSRS Wiki API."""
    # Try /Strategies subpage first, then base page
    titles_to_try = [f"{boss_name}/Strategies", boss_name]
    session = http_session  # persistent session
    for title in titles_to_try:
        ...
        try:
            async with session.get(WIKI_API_URL, params=params, headers=WIKI_HEADERS, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                if resp.status != 200:
                    continue
                data = await resp.json()
...
def parse_combat_styles_from_wikitext(wikitext: str) -> list[str]:
    """Extract which combat style tabs exist in the wikitext (e.g. Magic, Ranged, Melee)."""
    import re
    styles = []
    # Wiki uses {{Tab|Magic}}, {{tab|Ranged}} etc. in equipment/strategy sections
    tab_pattern = re.compile(r'\{\{[Tt]ab\|([^}|]+)', re.IGNORECASE)
    found = tab_pattern.findall(wikitext)
    seen = set()
    for s in found:
        s = s.strip()
        if s not in seen and any(k in s for k in ["Magic", "Ranged", "Melee"]):
            seen.add(s)
            styles.append(s)
    # If no tabs found, check for direct section headers like ==Magic==
    if not styles:
        header_pattern = re.compile(r'=={1,3}\s*(Magic|Ranged|Melee)\s*=={1,3}', re.IGNORECASE)
        for m in header_pattern.finditer(wikitext):
...
async def fetch_gear_image(wiki_name: str, style: str) -> tuple[bytes | None, str | None]:
    """
    Use Playwright to screenshot the OSRS Wiki strategy page with the correct
    style tab selected. Returns (image_bytes, error_message).
    """
    try:
    ...
    url = f"https://oldschool.runescape.wiki/w/{wiki_name.replace(' ', '_')}/Strategies"

    try:
        async with async_playwright() as p:
            # Try to find Chromium executable — Railway/Nix may place it in various locations
            import shutil
            import os as _os
            # Check env var first (set by Dockerfile), then fallback paths
            chromium_paths = [
            ...
            executable = next((p for p in chromium_paths if p and _os.path.exists(p)), None)
            ...
            # Block ads/trackers for faster load
            await page.route("**/*", lambda route: route.abort()
                if any(x in route.request.url for x in ["google", "analytics", "ads", "facebook"])
                else route.continue_()
            )

            await page.goto(url, wait_until="domcontentloaded", timeout=20000)

            # ── Click the correct style tab ───────────────────────────────────
            # Wiki tabs are rendered as <li> elements with the style name
            base_style = style.split("(")[0].strip()  # "Magic (Diary kill)" -> "Magic"
            tab_clicked = False
            try:
                # Try clicking the tab by text content
                tab = page.locator(f"li:has-text('{base_style}')").first
            ...
            if not tab_clicked:
                # Try by the exact tab label including suffix e.g. "Magic (Diary kill)"
                try:
                    tab = page.locator(f"li:has-text('{style}')").first
                    if await tab.is_visible():
                        await tab.click()
                        await page.wait_for_timeout(500)
                except Exception:
            ...
            # ── Find and screenshot the equipment section ─────────────────────
            # Wait for page to settle after tab click
            await page.wait_for_timeout(800)

            equipment_section = None

            # Strategy: find a table that has a "Slot" header — that's the gear table
            # Try multiple approaches in order of specificity
            selectors_to_try = [
                # Table with a Slot column header — most specific
                "table:has(th:has-text('Slot'))",
                # Table inside the active tab content
                ".wds-tab__content.wds-is-current table.wikitable",
                ".tabber__panel.tabber__panel--active table",
                # Any wikitable after an Equipment heading
                "h2 span#Equipment ~ * table",
                ...
                try:
                    ...
                    for i in range(count):
                        ...
                        if not await el.is_visible():
                            continue
                        # Check if this table has a Slot column (gear table indicator)
                        text = await el.inner_text()
                        if "Slot" in text and any(
                            slot in text for slot in ["Head", "Neck", "Cape", "Body", "Weapon", "Feet", "Ring"]
                        ):
                            equipment_section = el
                            break
            ...
            # Fallback: screenshot everything from the Equipment heading down
            if equipment_section is None:
                try:
                    # Find the Equipment section heading and screenshot from there
                    heading = page.locator("h2:has-text('Equipment'), h3:has-text('Equipment')").first
                    if await heading.is_visible():
                        # Get bounding box of heading, screenshot the area below it
                        box = await heading.bounding_box()
                        if box:
                            await page.screenshot(
                                type="png",
                                clip={"x": 0, "y": box["y"], "width": 1280, "height": 600}
                            )
                except Exception:
                    pass

            if equipment_section is None:
                # Last resort: full content area
                equipment_section = page.locator(".mw-parser-output").first

            # Scroll into view and screenshot
            await equipment_section.scroll_into_view_if_needed()
            await page.wait_for_timeout(300)
            img_bytes = await equipment_section.screenshot(type="png")
            await browser.close()
...
class GearStyleView(discord.ui.View):
    """Buttons for each available combat style for a boss."""

    def __init__(self, boss_name: str, wiki_name: str, styles: list[str], wikitext: str):
        ...
        for style in styles:
            emoji = STYLE_EMOJI.get(style, "⚔️")
            btn = discord.ui.Button(
                label=style,
                emoji=emoji,
                style=discord.ButtonStyle.primary,
                custom_id=f"gear_{wiki_name}_{style}"
            )
            btn.callback = self._make_callback(style)
            self.add_item(btn)

    def _make_callback(self, style: str):
        async def callback(interaction: discord.Interaction):
            if self._loading:
                await interaction.response.send_message("⏳ Already fetching — please wait!", ephemeral=True)
                return
            self._loading = True

            cache_key = f"{self.wiki_name.lower()}_{style.lower()}"
            now_ts = datetime.now().timestamp()

            # Check cache
            cached = gear_cache.get(cache_key)
            if cached and (now_ts - cached.get("fetched_at", 0)) < GEAR_CACHE_TTL:
                img_bytes = cached["data"]
                f = discord.File(fp=__import__("io").BytesIO(img_bytes), filename="gear.png")
                embed = discord.Embed(
                    title=f"⚔️  {self.wiki_name} — {style} Gear",
                    color={"Magic": 0x3498db, "Ranged": 0x2ecc71}.get(style.split()[0], 0xe74c3c)
                )
                embed.set_image(url="attachment://gear.png")
                embed.set_footer(text=f"Sourced from the OSRS Wiki • Avera Clan")
            ...
            _, img_bytes, err = await detect_gear_tabs_and_screenshot(self.wiki_name, style)
            print(f"[GEAR] {self.wiki_name} {style}: img_bytes={bool(img_bytes)}, err={err}")
            if err or not img_bytes:
                wiki_url = f"https://oldschool.runescape.wiki/w/{self.wiki_name.replace(' ', '_')}/Strategies"
                await interaction.followup.send(
                    f"⚠️ Couldn't build gear image right now (`{err}`).\n"
                    f"View it directly on the wiki: {wiki_url}"
                )
                self._loading = False
                return

            # Cache it
            gear_cache[cache_key] = {"data": img_bytes, "fetched_at": now_ts}

            import io as _io
            f = discord.File(fp=_io.BytesIO(img_bytes), filename="gear.png")
            colour_map = {"Magic": 0x3498db, "Ranged": 0x2ecc71, "Melee": 0xe74c3c}
            colour = colour_map.get(style.split()[0], 0xe74c3c)
            wiki_url = f"https://oldschool.runescape.wiki/w/{self.wiki_name.replace(' ', '_')}/Strategies"
            embed = discord.Embed(
                title=f"⚔️  {self.wiki_name} — {style} Gear",
                description=f"[View full strategy page]({wiki_url})",
                color=colour
            )
            embed.set_image(url="attachment://gear.png")
            embed.set_footer(text="Sourced from the OSRS Wiki • Avera Clan")
...
# ── GE Panel Refresh Button ───────────────────────────────────────────────────

_flip_refresh_cooldown = None  # datetime of last button-triggered refresh

class FlipRefreshView(discord.ui.View):
    """Persistent view that lives on the guide embed — provides a manual refresh button
    with a 1-minute global cooldown so it can't be spammed."""
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.primary,
        custom_id="flip_panel_refresh_btn"
    )
    async def refresh_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        global _flip_refresh_cooldown

        now = datetime.now()

        # ── Cooldown check ────────────────────────────────────────────
        if _flip_refresh_cooldown is not None:
            elapsed = (now - _flip_refresh_cooldown).total_seconds()
            remaining = 60 - elapsed
            if remaining > 0:
                await interaction.response.send_message(
                    f"⏳ Panel was just refreshed — please wait **{int(remaining)+1}s** before refreshing again.",
                    ephemeral=True
                )
                return
        ...
        _flip_refresh_cooldown = now

        try:
            await flip_panel_refresh()
            await interaction.followup.send(
                "✅ Panel refreshed with the latest prices!",
                ephemeral=True
            )
        except Exception as e:
            await interaction.followup.send(
...
async def build_flip_panel_embeds():
    """Build all embeds for the live GE flip panel — 4 categories + guide."""
    prices_raw, volumes_raw, prices_5m, prices_24h, name_map = await fetch_ge_prices()
    DISCLAIMER = "⚠️ *Verify by buying/selling a small amount first — prices shift fast. Use at your own risk.*"
    now        = datetime.now()
    unix_now   = int(now.timestamp())
    FOOTER     = "📡 prices.runescape.wiki"
    UPDATED_AT = f"\n\n🕐 Last updated: <t:{unix_now}:R>"

    def fmt_exact(amount):
        """Exact comma-formatted GP amount e.g. 10,450 gp"""
        try:
            return f"{int(amount):,} gp"
        except (TypeError, ValueError):
            return "N/A"

    def safe_lines(embed, lines):
        """Append item lines to embed description, guarding against Discord 4096 char limit."""
        result = []
        running = len(embed.description or "") + 2
        for line in lines:
    ...
    api_ok = bool(prices_raw and name_map)

    # ─────────────────────────────────────────────────────────
    # 1. BEST MARGINS
    # ─────────────────────────────────────────────────────────
    margins_embed = discord.Embed(
        title="💰  Best Margins",
        description=(
            f"> Highest profit per flip after 1% GE tax.\n"
            f"> Min **{format_gp(MIN_MARGIN)}** margin  ·  Min **{MIN_VOLUME:,}** trades/6h\n"
            f"> 🎯 Offer prices = 5-min avg **±1gp** for faster fills\n\n"
            + DISCLAIMER
            + UPDATED_AT
        ),
    ...
    if api_ok:
        candidates = []
        for item_id in (set(prices_raw.keys()) | (set(prices_5m.keys()) if prices_5m else set())):
            high, low, used_5m = get_best_prices(item_id, prices_5m, prices_raw)
            if not high or not low:
                continue
            margin = calc_margin(high, low)
            vol = int(volumes_raw.get(item_id, 0)) if volumes_raw else 0
            if margin < MIN_MARGIN or vol < MIN_VOLUME:
                continue
            name = name_map.get(item_id, f"Item #{item_id}")
            suggest_buy  = low  + 1
            suggest_sell = high - 1
            candidates.append((item_id, name, high, low, suggest_buy, suggest_sell, margin, vol))
        candidates.sort(key=lambda x: x[6], reverse=True)
        lines = []
        for i, (item_id, name, high, low, sbuy, ssell, margin, vol) in enumerate(candidates[:TOP_MARGINS_COUNT], 1):
            roi = (margin / low * 100) if low else 0
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
                f"{medal} **{name}**\n"
                f"🟢 Buy: **{sbuy:,} gp**  ·  🔴 Sell: **{ssell:,} gp**\n"
                f"📊 **{format_gp(margin)}** margin  ·  📈 {roi:.1f}% ROI  ·  🔄 {vol:,}/6h{limit_str}"
            )
        safe_lines(margins_embed, lines)
    else:
        margins_embed.description += "\n\n⚠️ Could not reach the OSRS Wiki API."
    margins_embed.set_footer(text=FOOTER)
    margins_embed.timestamp = now

    # ─────────────────────────────────────────────────────────
    # 2. HIGH VOLUME
    # ─────────────────────────────────────────────────────────
    hv_embed = discord.Embed(
        title="⚡  High Volume Flips",
        description=(
            f"> Fastest-moving items — great for flipping large quantities quickly.\n"
            f"> Min **{format_gp(HV_MIN_MARGIN)}** margin  ·  Min **{HV_MIN_VOLUME:,}** trades/6h\n"
            f"> 🎯 Offer prices = 5-min avg **±1gp** for faster fills\n\n"
            + DISCLAIMER
            + UPDATED_AT
        ),
    ...
    if api_ok:
        ...
        for item_id in (set(prices_raw.keys()) | (set(prices_5m.keys()) if prices_5m else set())):
            ...
            margin = calc_margin(high, low)
            vol = int(volumes_raw.get(item_id, 0)) if volumes_raw else 0
            if margin < HV_MIN_MARGIN or vol < HV_MIN_VOLUME:
                continue
            name = name_map.get(item_id, f"Item #{item_id}")
            suggest_buy  = low  + 1
            suggest_sell = high - 1
            hv_candidates.append((item_id, name, high, low, suggest_buy, suggest_sell, margin, vol))
        hv_candidates.sort(key=lambda x: x[7], reverse=True)
        lines = []
        for i, (item_id, name, high, low, sbuy, ssell, margin, vol) in enumerate(hv_candidates[:TOP_MARGINS_COUNT], 1):
            roi = (margin / low * 100) if low else 0
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 {buy_limit:,}/4h" if buy_limit else ""
        ...
        safe_lines(hv_embed, lines)
    else:
        hv_embed.description += "\n\n⚠️ Could not reach the OSRS Wiki API."
    hv_embed.set_footer(text=FOOTER)
    hv_embed.timestamp = now

    # ─────────────────────────────────────────────────────────
    # 3. TRENDING UP
    # ─────────────────────────────────────────────────────────
    up_embed = discord.Embed(
        title="📈  Trending Up  —  Past 24 Hours",
        description=(
            f"> Biggest price **increases** vs 24h average.\n"
            f"> Min **{TREND_MIN_VOLUME:,}** trades/6h  ·  Min price **{format_gp(TREND_MIN_PRICE)}**  ·  Min **{TREND_MIN_CHANGE:.0f}%** move\n\n"
            + DISCLAIMER
            + UPDATED_AT
        ),
        color=discord.Color.from_rgb(0, 220, 120)
    )
    if api_ok and prices_24h:
        up_candidates = []
        for item_id in (set(prices_raw.keys()) | (set(prices_5m.keys()) if prices_5m else set())):
            current_high, _, _ = get_best_prices(item_id, prices_5m, prices_raw)
            if not current_high or current_high < TREND_MIN_PRICE:
                continue
            hist = prices_24h.get(item_id, {})
            avg_high = hist.get("avgHighPrice")
            if not avg_high or avg_high == 0:
                continue
            pct_change = ((current_high - avg_high) / avg_high) * 100
            if pct_change < TREND_MIN_CHANGE:
                continue
            vol = int(volumes_raw.get(item_id, 0)) if volumes_raw else 0
            if vol < TREND_MIN_VOLUME:
                continue
            name = name_map.get(item_id, f"Item #{item_id}")
            gp_change = current_high - avg_high
            up_candidates.append((item_id, name, current_high, avg_high, pct_change, gp_change, vol))
        up_candidates.sort(key=lambda x: x[4], reverse=True)
        lines = []
        for i, (item_id, name, current, avg, pct, gp_chg, vol) in enumerate(up_candidates[:TOP_MARGINS_COUNT], 1):
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
                f"{medal} **{name}**\n"
                f"💰 Now: **{current:,} gp**  ·  📊 24h avg: {avg:,} gp\n"
                f"🚀 **+{pct:.1f}%**  (+{format_gp(gp_chg)})  ·  🔄 {vol:,}/6h{limit_str}"
            )
        safe_lines(up_embed, lines)
    elif not prices_24h:
        up_embed.description += "\n\n⚠️ 24h price data temporarily unavailable."
    else:
        up_embed.description += "\n\n⚠️ Could not reach the OSRS Wiki API."
    up_embed.set_footer(text=FOOTER)
    up_embed.timestamp = now

    # ─────────────────────────────────────────────────────────
    # 4. TRENDING DOWN
    # ─────────────────────────────────────────────────────────
    down_embed = discord.Embed(
        title="📉  Trending Down  —  Past 24 Hours",
        description=(
            f"> Biggest price **decreases** vs 24h average.\n"
            f"> Min **{TREND_MIN_VOLUME:,}** trades/6h  ·  Min price **{format_gp(TREND_MIN_PRICE)}**  ·  Min **{TREND_MIN_CHANGE:.0f}%** move\n\n"
            + DISCLAIMER
            + UPDATED_AT
        ),
    ...
    if api_ok and prices_24h:
        ...
        for item_id in (set(prices_raw.keys()) | (set(prices_5m.keys()) if prices_5m else set())):
            ...
            name = name_map.get(item_id, f"Item #{item_id}")
            gp_change = current_high - avg_high
            down_candidates.append((item_id, name, current_high, avg_high, pct_change, gp_change, vol))
        down_candidates.sort(key=lambda x: x[4])  # Most negative first
        lines = []
        for i, (item_id, name, current, avg, pct, gp_chg, vol) in enumerate(down_candidates[:TOP_MARGINS_COUNT], 1):
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
        ...
        safe_lines(down_embed, lines)
    elif not prices_24h:
        down_embed.description += "\n\n⚠️ 24h price data temporarily unavailable."
    else:
        down_embed.description += "\n\n⚠️ Could not reach the OSRS Wiki API."
    down_embed.set_footer(text=FOOTER)
    down_embed.timestamp = now

    # ─────────────────────────────────────────────────────────
    # 5. GUIDE
    # ─────────────────────────────────────────────────────────
    guide_embed = discord.Embed(
        title="📖  GE Flipping — Command Guide",
        color=discord.Color.from_rgb(255, 200, 50),
        description=(
            "**🔍 Price Check**\n"
            "`!price <item>` — Live buy/sell price, margin & 6h volume\n"
            "` ` Example: `!price Dragon bones`\n\n"
            "**🧮 Tax Calculator**\n"
            "`!tax <sell price> [qty]` — Net proceeds after 1% GE tax\n"
            "` ` Example: `!tax 500000 100`\n\n"
            "**📒 Flip Tracker**\n"
            "`!logflip <item> <buy> <sell> [qty]` — Log a flip\n"
            "`!myflips` — Your history & total profit\n"
            "`!clearflips` — Clear your history\n"
            "` ` Example: `!logflip Dragon bones 2000 2500 1000`\n\n"
            "**📊 Panel Categories**\n"
            "💰 **Best Margins** — highest profit per flip\n"
            "⚡ **High Volume** — fastest-moving items\n"
            "📈 **Trending Up** — biggest risers in 24h\n"
            "📉 **Trending Down** — biggest fallers in 24h\n\n"
            "*Prices from OSRS Wiki Prices API — updated in real time.*"
        )
    )
    guide_embed.set_footer(text="⚠️ Always verify before committing large quantities. | Avera Clan")

    refresh_view = FlipRefreshView()
    return margins_embed, hv_embed, up_embed, down_embed, guide_embed, refresh_view



# ── Live panel refresh task ──────────────────────────────────

@tasks.loop(minutes=15)
async def flip_panel_refresh():
    global flip_panel_message_id, flip_hv_message_id, flip_up_message_id, flip_down_message_id, flip_guide_message_id
    channel = bot.get_channel(FLIP_CHANNEL_ID)
    if not channel:
        return

    margins_embed, hv_embed, up_embed, down_embed, guide_embed, refresh_view = await build_flip_panel_embeds()

    # Map each stored message ID to its new embed
    id_embed_pairs = [
        ("flip_panel_message_id",  flip_panel_message_id,  margins_embed),
        ("flip_hv_message_id",     flip_hv_message_id,     hv_embed),
        ("flip_up_message_id",     flip_up_message_id,     up_embed),
        ("flip_down_message_id",   flip_down_message_id,   down_embed),
        ("flip_guide_message_id",  flip_guide_message_id,  guide_embed),
    ]

    all_ok = True
    for attr, msg_id, embed in id_embed_pairs:
        if msg_id:
            try:
                msg = await channel.fetch_message(msg_id)
                # Attach refresh view only to the guide message
                kw = {"view": refresh_view} if attr == "flip_guide_message_id" else {}
                await msg.edit(embed=embed, **kw)
                await asyncio.sleep(1.5)  # Avoid Discord rate limits on rapid PATCH requests
            except (discord.NotFound, discord.HTTPException):
                # Reset the specific ID that failed using explicit assignment
                if   attr == "flip_panel_message_id":  flip_panel_message_id  = None
                elif attr == "flip_hv_message_id":     flip_hv_message_id     = None
                elif attr == "flip_up_message_id":     flip_up_message_id     = None
                elif attr == "flip_down_message_id":   flip_down_message_id   = None
                elif attr == "flip_guide_message_id":  flip_guide_message_id  = None
                all_ok = False
        else:
            all_ok = False

    if all_ok:
        return  # All 5 messages edited successfully

    # One or more missing — purge bot messages and repost all fresh
    try:
        await channel.purge(limit=25, check=lambda m: m.author == channel.guild.me)
    except discord.Forbidden:
        pass

    # Post in order: guide first (pinned at top), then data embeds below
    g_msg    = await channel.send(embed=guide_embed, view=refresh_view)
    m_msg    = await channel.send(embed=margins_embed)
    hv_msg   = await channel.send(embed=hv_embed)
    up_msg   = await channel.send(embed=up_embed)
    down_msg = await channel.send(embed=down_embed)

    flip_guide_message_id  = g_msg.id
    flip_panel_message_id  = m_msg.id
    flip_hv_message_id     = hv_msg.id
    flip_up_message_id     = up_msg.id
    flip_down_message_id   = down_msg.id
    save_data()


@flip_panel_refresh.before_loop
async def before_flip_panel_refresh():
    await bot.wait_until_ready()


@flip_panel_refresh.error
async def flip_panel_refresh_error(error):
    """Log errors and keep the task running rather than silently dying."""
    print(f"⚠️ flip_panel_refresh error: {error}")
    log_channel = bot.get_channel(LOG_CHANNEL_ID)
    if log_channel:
        await log_channel.send(f"⚠️ **GE flip panel refresh failed:** `{error}`\nWill retry in 15 minutes.")
    flip_panel_refresh.restart()


# ── !flippanel — post/reset the panel manually ───────────────

@bot.command(name="flippanel")
@commands.has_permissions(manage_guild=True)
async def flip_panel(ctx):
    """Post or refresh the GE flipping panel. Admin only."""
    global flip_panel_message_id, flip_hv_message_id, flip_up_message_id, flip_down_message_id, flip_guide_message_id
    flip_panel_message_id = None
    flip_hv_message_id    = None
    flip_up_message_id    = None
    flip_down_message_id  = None
    flip_guide_message_id = None
    save_data()
    # Trigger the task immediately
    channel = bot.get_channel(FLIP_CHANNEL_ID)
    if not channel:
        await ctx.send("❌ Could not find the flip channel. Check FLIP_CHANNEL_ID.")
        return
    await flip_panel_refresh()
    if ctx.channel.id != FLIP_CHANNEL_ID:
        await ctx.send(f"✅ GE flip panel posted in <#{FLIP_CHANNEL_ID}>!")
    try:
        await ctx.message.delete()
    except discord.NotFound:
        pass
...
async def detect_gear_tabs_and_screenshot(wiki_name: str, style: str = None) -> tuple[list[str], bytes | None, str | None]:
    """
    Open the wiki strategy page with Playwright.
    - Detect which combat style tabs actually exist on the page.
    - If style is given, click that tab and screenshot the gear table.
    - If no tabs exist, screenshot the gear table directly.
    Returns (styles_found, image_bytes_or_None, error_or_None)
    """
    try:
        from playwright.async_api import async_playwright
    except ImportError:
    ...
    try:
        async with async_playwright() as p:
            ...
            executable = next((x for x in chromium_paths if x and _os.path.exists(x)), None)
            launch_kwargs = {
                "headless": True,
            ...
            page = await browser.new_page(viewport={"width": 1280, "height": 900})
            await page.route("**/*", lambda route: route.abort()
                if any(x in route.request.url for x in ["google-analytics", "googletagmanager", "facebook"])
                else route.continue_()
            )
            ...
            # ── Detect style tabs ─────────────────────────────────────────────
            styles_found = []
            STYLE_KEYWORDS = ["Magic", "Ranged", "Melee"]
            tab_selectors = [
                ".tabber__tab",
                ".wds-tabs__tab",
                "li.wds-tabs__tab",
                "nav.tabber__navigation a",
            ]
            for sel in tab_selectors:
                ...
                if count > 0:
                    for i in range(count):
                        ...
                        if any(kw in text for kw in STYLE_KEYWORDS):
                            if text not in styles_found:
                                styles_found.append(text)
                    if styles_found:
                        break

            # ── If just detecting tabs (no style requested), return early ─────
            if style is None:
                await browser.close()
                return styles_found, None, None

            # ── Click the requested style tab if tabs exist ───────────────────
            if styles_found and style:
                base_style = style.split("(")[0].strip()
                for sel in tab_selectors:
                    try:
                        tab = page.locator(f"{sel}:has-text('{base_style}')").first
                        if await tab.is_visible():
                            await tab.click()
            ...
            # Look for table with Slot column + gear slot names
            all_tables = page.locator("table.wikitable, table.wikitable-1, table.wikitable-2")
            ...
            for i in range(count):
                ...
                try:
                    text = await el.inner_text()
                    if "Slot" in text and any(
                        s in text for s in ["Head", "Neck", "Cape", "Body", "Weapon", "Feet", "Ring"]
                    ):
                        equipment_section = el
                        break
                except Exception:
            ...
            if equipment_section is None:
                # Fallback: first visible wikitable
                for i in range(count):
                    el = all_tables.nth(i)
            ...
            img_bytes = await equipment_section.screenshot(type="png")
            await browser.close()
            return styles_found, img_bytes, None
...
async def gear_command(ctx, *, boss: str = None):
    """Look up recommended gear for any OSRS boss. Usage: !gear <boss name>
    Example: !gear Zulrah"""
    global gear_cache

    if not boss:
        await ctx.send("❌ Usage: `!gear <boss name>`\nExample: `!gear Zulrah`")
        return

    boss_lower = boss.lower().strip()

    # Resolve alias → wiki name
    wiki_name = BOSS_ALIASES.get(boss_lower)
    if not wiki_name:
        for alias, name in BOSS_ALIASES.items():
            if boss_lower in alias or alias in boss_lower:
                wiki_name = name
                break
    if not wiki_name:
        wiki_name = boss.strip().title()

    thinking_msg = await ctx.send(f"🔍 Looking up **{wiki_name}**...")

    # Use Playwright to detect actual tabs on the wiki page
    cache_key_tabs = f"{wiki_name.lower()}_tabs"
    now_ts = datetime.now().timestamp()

    cached_tabs = gear_cache.get(cache_key_tabs)
    if cached_tabs and (now_ts - cached_tabs.get("fetched_at", 0)) < GEAR_CACHE_TTL:
        styles = cached_tabs["data"]
    else:
        styles, _, err = await detect_gear_tabs_and_screenshot(wiki_name, style=None)
        gear_cache[cache_key_tabs] = {"data": styles, "fetched_at": now_ts}

    if not styles:
        # No style tabs found — screenshot gear section directly
        await thinking_msg.edit(content=f"📸 Fetching gear for **{wiki_name}**...")
        cache_key = f"{wiki_name.lower()}_all"
        cached = gear_cache.get(cache_key)
        if cached and (now_ts - cached.get("fetched_at", 0)) < GEAR_CACHE_TTL:
            img_bytes = cached["data"]
        else:
            _, img_bytes, err = await detect_gear_tabs_and_screenshot(wiki_name, style="")
            if not img_bytes:
                wiki_url = f"https://oldschool.runescape.wiki/w/{wiki_name.replace(' ', '_')}/Strategies"
                await thinking_msg.edit(content=f"⚠️ Couldn't fetch gear for **{wiki_name}**. View it here: {wiki_url}")
                return
            gear_cache[cache_key] = {"data": img_bytes, "fetched_at": now_ts}
        ...
        wiki_url = f"https://oldschool.runescape.wiki/w/{wiki_name.replace(' ', '_')}/Strategies"
        embed = discord.Embed(
            title=f"⚔️  {wiki_name} — Recommended Gear",
            description=f"[View full strategy page]({wiki_url})",
            color=discord.Color.gold()
        )
        embed.set_image(url="attachment://gear.png")
        embed.set_footer(text="Sourced from the OSRS Wiki • Avera Clan")
        await thinking_msg.edit(content=None, embed=embed, attachments=[f])
        return

    # Style tabs found — show buttons
    style_list = " / ".join(f"{STYLE_EMOJI.get(s, '⚔️')} {s}" for s in styles)
    embed = discord.Embed(
        title=f"⚔️  {wiki_name} — Gear Guide",
        description=(
            f"Choose a combat style to see the full recommended gear setup.\n\n"
            f"**Available styles:** {style_list}"
        ),
        color=discord.Color.gold()
    )
    embed.set_footer(text="Sourced from the OSRS Wiki • Avera Clan")
    view = GearStyleView(boss_lower, wiki_name, styles, "")
    await thinking_msg.edit(content=None, embed=embed, view=view)



@bot.command(name="price")
async def ge_price(ctx, *, item_name: str = None):
    """Look up the live GE price for an item.
    Usage: !price <item name>
    Example: !price Dragon bones"""
    if not item_name:
        await ctx.send("❌ Usage: `!price <item name>`\nExample: `!price Dragon bones`")
        return

    async with ctx.typing():
        result = await fetch_item_price(item_name)

    if not result:
        await ctx.send(
            f"❌ Couldn't find **{item_name}** on the GE.\n"
            f"Check the spelling and try again — item names are case-insensitive."
        )
        return

    # Handle item found but no current price data
    if result["high"] is None or result["low"] is None:
        await ctx.send(
            f"⚠️ **{result['name']}** was found but has no current price data on the GE.\n"
            f"It may not be actively traded right now."
        )
        return

    avg_high      = result["high"]           # 5m avg high
    avg_low       = result["low"]            # 5m avg low
    suggest_buy   = result["suggest_buy"]    # avg_low  + 1gp
    suggest_sell  = result["suggest_sell"]   # avg_high - 1gp
    last_high     = result.get("last_high")  # /latest snapshot
    last_low      = result.get("last_low")
    margin        = result["margin"]
    vol           = result["volume"]
    used_5m       = result.get("used_5m", False)
    roi           = (margin / avg_low * 100) if avg_low else 0
    tax           = min(int(suggest_sell * GE_TAX_RATE), GE_TAX_CAP)

    color = discord.Color.from_rgb(50, 205, 50) if margin > 0 else discord.Color.from_rgb(220, 60, 60)
    data_label = "5m avg" if used_5m else "/latest"
    embed = discord.Embed(
        title=f"🔍  {result['name']}",
        description=f"*Prices based on {data_label} — {'smoothed over last 5 minutes' if used_5m else 'most recent single transaction'}*",
        color=color
    )
    buy_limit = ge_item_limit_map.get(result["id"])
    # Suggested offer prices — the key actionable numbers
    embed.add_field(name="🎯 Suggested Buy Offer",  value=f"**{suggest_buy:,} gp**",   inline=True)
    embed.add_field(name="🎯 Suggested Sell Offer", value=f"**{suggest_sell:,} gp**",  inline=True)
    embed.add_field(name="💸 GE Tax",               value=f"{tax:,} gp",               inline=True)
    embed.add_field(name="📊 Margin (after tax)",   value=f"**{format_gp(margin)}**",  inline=True)
    embed.add_field(name="📈 ROI",                  value=f"{roi:.1f}%",               inline=True)
    embed.add_field(name="🔄 Volume (6h)",          value=f"{vol:,}",                  inline=True)
    if buy_limit:
        embed.add_field(name="🛒 Buy Limit (4h)",   value=f"**{buy_limit:,}**",        inline=True)
    # Last trade reference — smaller context
    if last_high and last_low:
        embed.add_field(
            name="🕐 Last Trade (snapshot)",
            value=f"Buy: {last_low:,} gp  ·  Sell: {last_high:,} gp",
            inline=False
        )
    embed.set_footer(text="🎯 Suggested offers = 5m avg ±1gp for faster fills  •  📡 prices.runescape.wiki")
    embed.timestamp = datetime.now()
    await ctx.send(embed=embed)


# ── !tax <sell_price> [quantity] ─────────────────────────────

@bot.command(name="tax")
async def ge_tax(ctx, sell_price: str = None, quantity: str = "1"):
    """Calculate GE tax and net proceeds.
    Usage: !tax <sell price> [quantity]
    Example: !tax 500000 100"""
    if not sell_price:
        await ctx.send("❌ Usage: `!tax <sell price> [quantity]`\nExample: `!tax 500000 100`")
        return

    try:
        sell = int(sell_price.replace(",", ""))
        qty  = int(quantity.replace(",", ""))
    except ValueError:
        await ctx.send("❌ Please enter valid numbers. Example: `!tax 500000 100`")
        return

    if sell <= 0 or qty <= 0:
        await ctx.send("❌ Sell price and quantity must be positive.")
        return

    tax_per    = min(int(sell * GE_TAX_RATE), GE_TAX_CAP)
    net_per    = sell - tax_per
    total_tax  = tax_per * qty
    total_net  = net_per * qty
    total_sell = sell * qty

    embed = discord.Embed(
        title="🧮 GE Tax Calculator",
        color=discord.Color.blue()
    )
    embed.add_field(name="💰 Sell Price (each)",  value=format_gp(sell),       inline=True)
    embed.add_field(name="💸 Tax (each)",         value=format_gp(tax_per),    inline=True)
    embed.add_field(name="✅ Net (each)",          value=format_gp(net_per),    inline=True)
    if qty > 1:
        embed.add_field(name="📦 Quantity",        value=f"{qty:,}",            inline=True)
        embed.add_field(name="💸 Total Tax",       value=format_gp(total_tax),  inline=True)
        embed.add_field(name="✅ Total Net",        value=format_gp(total_net),  inline=True)
    embed.set_footer(text="1% GE tax • Capped at 5,000,000 gp per item")
    await ctx.send(embed=embed)


# ── !logflip <item> <buy> <sell> <qty> ───────────────────────

@bot.command(name="logflip")
async def log_flip(ctx, *, args: str = None):
    """Log a completed GE flip to track your profit.
    Usage: !logflip <item name> <buy> <sell> [quantity]
    Example: !logflip Dragon bones 2000 2500 1000
    The last 2 or 3 space-separated tokens are numbers — everything before is the item name."""
    if not args:
        await ctx.send(
            "❌ Usage: `!logflip <item name> <buy> <sell> [qty]`\n"
            "Example: `!logflip Dragon bones 2000 2500 1000`\n"
            "The last 2–3 values are numbers; everything before is the item name."
        )
        return

    # Split from the right: last 3 tokens checked as numbers first (item buy sell qty),
    # then fall back to last 2 (item buy sell)
    tokens = args.split()
    buy, sell, qty = None, None, 1
    item = None
    ...
    if len(tokens) >= 4:
        ...
        s = try_int(tokens[-2])
        b = try_int(tokens[-3])
        if q is not None and s is not None and b is not None:
            qty, sell, buy = q, s, b
            item = " ".join(tokens[:-3])

    if item is None and len(tokens) >= 3:
        s = try_int(tokens[-1])
        b = try_int(tokens[-2])
    ...
    # Reject if item name is purely numeric (means user forgot the item name)
    if item and item.replace(",", "").replace(" ", "").lstrip("-").isdigit():
        item = None

    if not item or buy is None or sell is None:
        await ctx.send(
            "❌ Couldn't parse your input. Usage: `!logflip <item name> <buy> <sell> [qty]`\n"
            "Example: `!logflip Dragon bones 2000 2500 1000`"
        )
        return
    ...
    profit = calc_profit(buy, sell, qty)
    user_id = str(ctx.author.id)

    if user_id not in flip_logs:
        flip_logs[user_id] = []

    flip_logs[user_id].append({
        "item":      item,
        "buy":       buy,
        "sell":      sell,
        "quantity":  qty,
    ...
    # Cap flip log at 500 entries per user — drop oldest to protect MongoDB doc size
    if len(flip_logs[user_id]) > 500:
        flip_logs[user_id] = flip_logs[user_id][-500:]

    color = discord.Color.green() if profit >= 0 else discord.Color.red()
    embed = discord.Embed(
        title=f"{'📈' if profit >= 0 else '📉'} Flip Logged — {item}",
        color=color
    )
    embed.add_field(name="📥 Buy",       value=f"{format_gp(buy)} ea",     inline=True)
    embed.add_field(name="📤 Sell",      value=f"{format_gp(sell)} ea",    inline=True)
    embed.add_field(name="📦 Qty",       value=f"{qty:,}",                  inline=True)
    embed.add_field(name="💸 Tax",       value=format_gp(min(int(sell * GE_TAX_RATE), GE_TAX_CAP) * qty), inline=True)
    embed.add_field(name="💰 Net Profit",value=f"**{format_gp(profit)}**", inline=True)
    embed.add_field(name="📊 Total Flips", value=str(len(flip_logs[user_id])), inline=True)
    embed.set_footer(text=f"Logged by {ctx.author.display_name}")
    embed.timestamp = datetime.now()
    save_data()
    await ctx.send(embed=embed)
...
@bot.command(name="myflips")
async def my_flips(ctx, member: discord.Member = None):
    """View your GE flip history and total profit.
    Usage: !myflips [@member]"""
    target  = member or ctx.author
    user_id = str(target.id)
    logs    = flip_logs.get(user_id, [])

    if not logs:
        msg = "You haven't logged any flips yet!" if target == ctx.author else f"**{target.display_name}** hasn't logged any flips yet!"
        await ctx.send(f"📭 {msg} Use `!logflip` to get started.")
        return

    total_profit = sum(f["profit"] for f in logs)
    total_flips  = len(logs)
    best_flip    = max(logs, key=lambda f: f["profit"])
    recent       = logs[-5:]  # last 5

    color = discord.Color.green() if total_profit >= 0 else discord.Color.red()
    embed = discord.Embed(
        title=f"📊 {target.display_name}'s Flip Log",
        color=color
    )
    embed.add_field(name="🔢 Total Flips",  value=str(total_flips),            inline=True)
    embed.add_field(name="💰 Total Profit", value=f"**{format_gp(total_profit)}**", inline=True)
    all_losses = all(f["profit"] < 0 for f in logs)
    best_label = "📉 Least Bad Loss" if all_losses else "🏆 Best Flip"
    best_sign  = "" if all_losses else "+"
    embed.add_field(name=best_label, value=f"{best_flip['item']} ({best_sign}{format_gp(best_flip['profit'])})", inline=True)

    recent_lines = []
    for f in reversed(recent):
        sign = "+" if f["profit"] >= 0 else ""
        line = f"• **{f['item']}** x{f['quantity']:,} → {sign}{format_gp(f['profit'])} *({f['timestamp']})*"
        recent_lines.append(line)
    recent_value = "\n".join(recent_lines)
    # Truncate to Discord field value limit (1024 chars)
    if len(recent_value) > 1020:
        recent_value = recent_value[:1017] + "..."
    embed.add_field(name="🕒 Recent Flips (last 5)", value=recent_value, inline=False)
    embed.set_footer(text="Use !logflip to add a flip • !clearflips to reset")
    await ctx.send(embed=embed)


# ── !clearflips ──────────────────────────────────────────────

@bot.command(name="clearflips")
async def clear_flips(ctx):
    """Clear your personal flip log. Usage: !clearflips"""
    user_id = str(ctx.author.id)
    count   = len(flip_logs.get(user_id, []))
    if count == 0:
        await ctx.send("📭 You don't have any logged flips to clear.")
        return
    flip_logs[user_id] = []
    save_data()
    await ctx.send(f"🗑️ Cleared **{count}** flip{'s' if count != 1 else ''} from your log.")
    ...
    for attempt in range(5):
        try:
            ...
            if e.status == 429:
                ...
                print(f"⚠️ Discord login rate limited (429). Waiting {wait}s before retry {attempt+1}/5...")
                time.sleep(wait)
            else:
                raise
