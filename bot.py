...
def calculate_glory(user_id, total_level=None, ehp=None):
...
def get_rank(points, user_id=None, total_level=None, ehp=None):
    ...
    if uid and uid in member_ranks:
        return member_ranks[uid]
    glory = calculate_glory(uid, total_level, ehp)
...
async def fetch_osrs_stats(rsn):
    """Fetch total level and EHP from OSRS hiscores. Returns (total_level, ehp)."""
    total_level = None
    ehp = None
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
                    ...
                    if len(lines) > 52:
                        ehp_parts = lines[52].split(",")
                        if len(ehp_parts) > 1:
                            try:
                                ehp = float(ehp_parts[1])
                            except ValueError:
                                ehp = None
    except Exception:
        pass
...
async def rank_check(ctx, member: discord.Member = None):
    ...
    glory = calculate_glory(user_id, total_level, ehp)
...
async def open_promotion_ticket(interaction: discord.Interaction):
    ...
    stats_embed.add_field(
        ...
        value=(
            ...
            "total level, EHP, bounties and referrals.\n\n"
