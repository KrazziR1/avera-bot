...
def calculate_glory(user_id, total_level=None, ehp=None, collection_log=None, diaries=None):
    """Calculate a member's total Glory score from all inputs."""
    uid = str(user_id) if user_id else None
    glory = 0.0

    # Time in clan: 10 pts per month (fractional)
    months = get_months_since_verify(uid) if uid else 0
    glory += months * 10

    # Total level: 1 pt per 100 levels (granular)
    if total_level:
        glory += total_level / 100
    ...
    if uid:
        bounties_claimed = sum(
            ...
            if not b.get("active", True) and b.get("claimed_by_id") == uid
               or not b.get("active", True) and b.get("claimed_by") == (
                   None  # fallback handled below
               )
        )
        # More reliable count using claimed_by_id if available
        bounties_claimed = sum(
            1 for b in bounties.values()
    ...
    # Collection log slots: 1 pt per 50 slots
    if collection_log:
        glory += collection_log / 50

    # Achievement diaries
    if diaries:
        glory += diaries.get("easy", 0) * 1
        glory += diaries.get("medium", 0) * 2
        glory += diaries.get("hard", 0) * 4
        glory += diaries.get("elite", 0) * 8
...
def get_rank(points, user_id=None, total_level=None, ehp=None, collection_log=None, diaries=None):
    """Wrapper — calculates Glory and returns rank. Keeps old signature compatible."""
    uid = str(user_id) if user_id else None
    if uid and uid in member_ranks:
        return member_ranks[uid]
    glory = calculate_glory(uid, total_level, ehp, collection_log, diaries)
...
async def fetch_osrs_stats(rsn):
    """Fetch total level from OSRS hiscores and EHP from Temple OSRS. Returns (total_level, ehp) or (None, None)."""
    total_level = None
    ehp = None
    try:
        # Total level from hiscores
    ...
    try:
        # EHP from Temple OSRS (more reliable than hiscores index)
        temple_url = f"https://templeosrs.com/api/player_info.php?player={rsn.replace(' ', '%20')}"
        async with aiohttp.ClientSession() as session:
            async with session.get(temple_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
                    data = await resp.json(content_type=None)
                    info = data.get("data", {})
                    ehp_val = info.get("Ehp") or info.get("EHP") or info.get("ehp")
                    if ehp_val is not None:
                        ehp = float(ehp_val)
    except Exception:
        pass
...
async def fetch_temple_stats(rsn):
    """Fetch collection log slots and diary completions from TempleOSRS API."""
    try:
        url = f"https://templeosrs.com/api/player_stats.php?player={rsn.replace(' ', '%20')}&bosses=1"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status != 200:
                    return None, None
        ...
        # Collection log slots — confirmed Temple OSRS field name
        collection_log = None
        for key in ["Collections", "Collection_log_count", "Collectionlog", "Collection_log"]:
            val = info.get(key)
            if val is not None:
                try:
                    collection_log = int(val)
                    break
        ...
        def get_diary(tier):
            candidates = [
                f"Diary_{tier}_complete",
                f"Diary_{tier.capitalize()}_complete",
                f"Achievement_diary_{tier}",
                f"Diaries_{tier}",
                f"diary_{tier}",
            ]
        ...
        diaries = {
            "easy":   get_diary("easy"),
            "medium": get_diary("medium"),
            "hard":   get_diary("hard"),
            "elite":  get_diary("elite"),
        }

        return collection_log, diaries
    except Exception:
        return None, None
...
async def rank_check(ctx, member: discord.Member = None):
    ...
    collection_log, diaries = await fetch_temple_stats(rsn)
    months = get_months_since_verify(user_id)
    glory = calculate_glory(user_id, total_level, ehp, collection_log, diaries)
    ...
    if collection_log:
        stats_text += f"📖 **Collection Log:** {collection_log} slots\n"
...
async def glory_command(ctx, member: discord.Member = None):
    ...
    # Build breakdown
    time_glory = round(months * 10, 1)
    level_glory = round((total_level or 0) / 100, 1)
    ehp_glory = round((ehp or 0) / 50, 1)
    ...
    log_glory = round((collection_log or 0) / 50, 1)
    diary_glory = 0
    if diaries:
        diary_glory = diaries.get("easy",0)*1 + diaries.get("medium",0)*2 + diaries.get("hard",0)*4 + diaries.get("elite",0)*8
    ...
    embed.add_field(
        ...
        value=(
            ...
            f"📊 Total Level ({total_level or 0:,}): **+{level_glory}**\n"
            f"⚡ EHP ({ehp or 0:.0f}): **+{ehp_glory}**\n"
            f"🎯 Bounties claimed ({bounties_claimed}): **+{bounty_glory}**\n"
            f"👥 Referrals ({ref_count}): **+{ref_glory}**\n"
            f"📖 Collection log ({collection_log or 0} slots): **+{log_glory}**\n"
            f"📜 Diaries: **+{diary_glory}**"
        ),
        inline=False
    )
...
async def rank_progress(ctx, member: discord.Member = None):
    ...
    months_needed = round(needed / 10, 1)
    suggestions.append(f"⏱️ Stay active for **{months_needed} more months** (+{months_needed*10:.0f} Glory)")
    if (total_level or 0) < 2376:
        level_needed = int(needed * 100)
        suggestions.append(f"📊 Gain **{level_needed} more total levels** (+{level_needed/100:.1f} Glory)")
    suggestions.append(f"👥 Refer **{max(1, int(needed/10))} new member(s)** (+{max(1,int(needed/10))*10:.0f} Glory)")
...
async def welcome_panel(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "in-game progression, bounties, diaries and more.\n\n"
...
@bot.command(name="templedebug")
@commands.has_permissions(manage_guild=True)
async def temple_debug(ctx, *, rsn: str = None):
    """Debug Temple OSRS API response for a player. Admin only. Usage: !templedebug <rsn>"""
    if not rsn:
        user_rsn = verified_rsns.get(str(ctx.author.id))
        if not user_rsn:
            await ctx.send("❌ Provide an RSN or verify yours first.")
            return
        rsn = user_rsn

    msg = await ctx.send(f"🔍 Fetching Temple OSRS data for **{rsn}**...")
    try:
        url = f"https://templeosrs.com/api/player_stats.php?player={rsn.replace(' ', '%20')}&bosses=1"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
        ...
        info = data.get("data", {})
        # Filter to diary and collection log related keys
        relevant = {k: v for k, v in info.items() if any(x in k.lower() for x in ["diary", "collection", "log", "achieve", "task", "quest"])}

        if relevant:
            lines = "\n".join(f"`{k}`: {v}" for k, v in list(relevant.items())[:20])
            await msg.edit(content=f"📊 Temple OSRS fields for **{rsn}**:\n{lines}")
        else:
            all_keys = "\n".join(f"`{k}`: {v}" for k, v in list(info.items())[:40])
            await msg.edit(content=f"⚠️ No diary/collection fields found. All fields:\n{all_keys}")
    except Exception as e:
        await msg.edit(content=f"❌ Error fetching Temple data: {e}")
...
async def faq_panel(ctx):
    ...
    embed3.add_field(
        ...
        value=(
            ...
            "📊 **Your in-game stats** — total level and EHP\n"
            "🎯 **Bounties claimed** — completing clan challenges\n"
            "📖 **Collection log progress** — breadth of your PvM experience\n"
            "📜 **Achievement diaries** — your progression across Gielinor\n"
    ...
    embed3.add_field(
        name="🔗 Sync with Temple OSRS",
        value=(
            "Collection log and diary data is pulled from **Temple OSRS** — a free, trusted OSRS tracking site.\n\n"
            "To make sure your Glory is fully accurate:\n"
            "1️⃣ Visit **templeosrs.com** and search your RSN\n"
            "2️⃣ Claim your profile and enable the **Temple OSRS** plugin in RuneLite\n"
            "3️⃣ Your data will sync automatically and your Glory will update next time you run `!glory`\n\n"
            "🔗 https://templeosrs.com"
        ),
        inline=False
    )
