...
gear_cache         = {}  # Boss gear cache: { "key": { data, fetched_at } }
...
async def help_command(ctx):
    ...
    embed.add_field(name="⚔️ Boss Guides", value=(
        f"`{PREFIX}gear <boss>` — Gear guide with wiki screenshot for any boss\n"
        f"` ` Example: `!gear Zulrah` · `!gear Vorkath` · `!gear Tombs of Amascut`"
    ), inline=False)
...
#  Uses Playwright to screenshot the OSRS Wiki strategy page with the correct
#  style tab selected. Detects available tabs automatically.
# ══════════════════════════════════════════════════════════════════════════════

WIKI_API_URL   = "https://oldschool.runescape.wiki/api.php"
GEAR_CACHE_TTL = 86400  # 24 hours

BOSS_ALIASES = {
    "zulrah": "Zulrah", "zul": "Zulrah",
    "vorkath": "Vorkath", "vork": "Vorkath",
    "corp": "Corporeal Beast", "corporeal": "Corporeal Beast",
    ...
    "zammy": "K'ril Tsutsaroth", "kril": "K'ril Tsutsaroth",
    "cerb": "Cerberus", "cerberus": "Cerberus",
    "hydra": "Alchemical Hydra",
    "kraken": "Kraken",
    "thermy": "Thermonuclear Smoke Devil",
    "dks": "Dagannoth Kings", "dag kings": "Dagannoth Kings",
    ...
    "gauntlet": "The Gauntlet", "cg": "Corrupted Gauntlet", "corrupted gauntlet": "Corrupted Gauntlet",
    "gg": "Grotesque Guardians", "grotesque": "Grotesque Guardians",
    "sire": "Abyssal Sire", "abyssal sire": "Abyssal Sire",
    "gorillas": "Demonic gorilla", "demonics": "Demonic gorilla",
    "kq": "Kalphite Queen", "kalphite queen": "Kalphite Queen",
...
STYLE_EMOJI = {
    "Magic": "🔵", "Ranged": "🟢", "Melee": "🔴",
    "Magic (Diary kill)": "📜", "Ranged (Diary kill)": "📜",
}


async def _playwright_gear(wiki_name: str, style: str | None) -> tuple[list, bytes | None, str | None]:
    """
    Open wiki strategy page with Playwright.
    If style is None: detect tabs only, return (styles, None, None).
    If style is given: click that tab (if exists) and screenshot gear table.
    Returns (styles_found, image_bytes, error).
    """
    try:
    ...
    try:
        async with async_playwright() as pw:
            ...
            exe_candidates = [
                ...
                _sh.which("chromium"), _sh.which("chromium-browser"),
            ]
            exe = next((x for x in exe_candidates if x and _os.path.exists(x)), None)
            launch_kw = {
                "headless": True,
            ...
            await page.route("**/*", lambda r: r.abort()
                if any(x in r.request.url for x in ["google-analytics", "googletagmanager"])
                else r.continue_())
            ...
            # ── Detect style tabs ─────────────────────────────────────────
            styles_found = []
            STYLE_KW = ["Magic", "Ranged", "Melee"]
            for sel in [".tabber__tab", ".wds-tabs__tab", "nav.tabber__navigation a", "li.wds-tabs__tab"]:
                ...
                for i in range(n):
                    ...
                    if any(k in txt for k in STYLE_KW) and txt not in styles_found:
                        styles_found.append(txt)
                if styles_found:
                    break
            ...
            if styles_found and style:
                base = style.split("(")[0].strip()
                for sel in [".tabber__tab", ".wds-tabs__tab", "nav.tabber__navigation a"]:
                    try:
                        tab = page.locator(f"{sel}:has-text('{base}')").first
            ...
            # ── Find gear table (has Slot column + gear slot names) ───────
            equipment = None
            tables = page.locator("table.wikitable, table.wikitable-bordered")
            ...
            for i in range(n):
                ...
                try:
                    ...
                    if "Slot" in txt and any(s in txt for s in ["Head","Neck","Cape","Body","Weapon","Feet","Ring"]):
                        equipment = el
                        break
                except Exception:
                    continue
            ...
            if equipment is None:
                # Fallback to first visible wikitable
                for i in range(n):
                    el = tables.nth(i)
            ...
            if equipment is None:
                equipment = page.locator(".mw-parser-output").first

            await equipment.scroll_into_view_if_needed()
            await page.wait_for_timeout(300)
            img = await equipment.screenshot(type="png")
            await browser.close()
            return styles_found, img, None
...
class GearStyleView(discord.ui.View):
    def __init__(self, wiki_name: str, styles: list):
        ...
        for style in styles:
            ...
            btn.callback = self._make_cb(style)
            self.add_item(btn)
    ...
    def _make_cb(self, style: str):
        async def cb(interaction: discord.Interaction):
            ...
            if cached and (now_ts - cached.get("fetched_at", 0)) < GEAR_CACHE_TTL:
                ...
                _, img, err = await _playwright_gear(self.wiki_name, style)
                if not img:
                    wiki_url = f"https://oldschool.runescape.wiki/w/{self.wiki_name.replace(' ', '_')}/Strategies"
                    await interaction.followup.send(
                        f"⚠️ Couldn't fetch gear right now (`{err}`).\nView it here: {wiki_url}")
                    self._loading = False
                    return
                gear_cache[cache_key] = {"data": img, "fetched_at": now_ts}

            import io as _io
            f     = discord.File(fp=_io.BytesIO(img), filename="gear.png")
            ...
            if hasattr(interaction, "_deferred") or True:
                try:
...
async def gear_command(ctx, *, boss: str = None):
    """Look up recommended gear for any OSRS boss. Usage: !gear <boss>"""
    global gear_cache
    if not boss:
    ...
    thinking = await ctx.send(f"🔍 Looking up **{wiki_name}**...")

    # Check tab cache
    now_ts          = datetime.now().timestamp()
    tab_cache_key   = f"{wiki_name.lower()}_tabs"
    cached_tabs     = gear_cache.get(tab_cache_key)
    if cached_tabs and (now_ts - cached_tabs.get("fetched_at", 0)) < GEAR_CACHE_TTL:
        styles = cached_tabs["data"]
    else:
        styles, _, _ = await _playwright_gear(wiki_name, None)
        gear_cache[tab_cache_key] = {"data": styles, "fetched_at": now_ts}

    if not styles:
        # No style tabs — screenshot full gear section directly
        img_cache_key = f"{wiki_name.lower()}_all"
        cached_img    = gear_cache.get(img_cache_key)
        if cached_img and (now_ts - cached_img.get("fetched_at", 0)) < GEAR_CACHE_TTL:
            img = cached_img["data"]
        else:
            await thinking.edit(content=f"📸 Fetching gear for **{wiki_name}**...")
            _, img, err = await _playwright_gear(wiki_name, "")
            if not img:
                wiki_url = f"https://oldschool.runescape.wiki/w/{wiki_name.replace(' ', '_')}/Strategies"
                await thinking.edit(content=f"⚠️ Couldn't fetch gear. View it here: {wiki_url}")
                return
            gear_cache[img_cache_key] = {"data": img, "fetched_at": now_ts}
        ...
        embed.set_image(url="attachment://gear.png")
        embed.set_footer(text="Sourced from the OSRS Wiki • Avera Clan")
        await thinking.edit(content=None, embed=embed, attachments=[f])
        return
    ...
    embed = discord.Embed(
        ...
        description=f"Choose a combat style:\n\n**Available:** {style_list}",
        color=discord.Color.gold()
    )
    embed.set_footer(text="Sourced from the OSRS Wiki • Avera Clan")
    await thinking.edit(content=None, embed=embed, view=GearStyleView(wiki_name, styles))
    ...
    for attr, msg_id, embed in id_embed_pairs:
        if msg_id:
            try:
                ...
                await asyncio.sleep(3.0)  # Avoid Discord rate limits on rapid PATCH requests
