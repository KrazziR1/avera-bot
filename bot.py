...
async def help_command(ctx):
    ...
    embed.add_field(name="📅 Events", value=(
        ...
        f"`{PREFIX}startattendance <#> [@host]` — Open attendance, optionally tag a host *(Admin only)*\n"
...
@commands.has_permissions(manage_guild=True)
async def start_attendance(ctx, event_number: int = None, host: discord.Member = None):
    """Start attendance tracking for an event. Admin only.
    Usage: !startattendance <event number> [@host]
    Example: !startattendance 1 @Jerzi"""
    ...
    if event_number is None:
        await ctx.send("❌ Please provide an event number. Example: `!startattendance 1`\nOptionally tag a host: `!startattendance 1 @Jerzi`\nUse `!events` to see the list.")
        return
    ...
    if host:
        host_id = str(host.id)
        points_data[host_id] = points_data.get(host_id, 0) + HOST_BONUS
        weekly_stats["points_awarded"] += HOST_BONUS
        weekly_stats["weekly_points"][host_id] = weekly_stats["weekly_points"].get(host_id, 0) + HOST_BONUS
        host_text = f"\n🌟 **Hosted by {host.display_name}** — +{HOST_BONUS} pts awarded for organising!"
    ...
    if ctx.channel.id != EVENTS_CHANNEL_ID:
        host_confirm = f" Host: **{host.display_name}** (+{HOST_BONUS} pts)" if host else " No host tagged."
...
async def verify_rsn(ctx, *, rsn: str = None):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            + (f"*(Previously linked to: {old_rsn})*" if old_rsn and old_rsn != rsn else "")
        ),
        color=discord.Color.green()
    )
