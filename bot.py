...
async def profile(ctx, member: discord.Member = None):
    ...
    if rsn:
        total_level, total_xp = await fetch_osrs_stats(rsn)
        if total_level is None or total_xp is None:
            # Hiscores failed — fall back to cache silently
            cached_glory = glory_data.get(user_id, {}).get("glory", 0)
    ...
    embed.set_footer(text="Avera Clan • Use !glory for full breakdown • !rankprogress for next rank tips")
    embed.timestamp = datetime.now()
    await ctx.send(embed=embed)
...
async def fetch_ge_prices():
    """Fetch latest GE prices, volumes, 24h averages and name map from OSRS Wiki.
    Returns (prices_raw, volumes_raw, prices_24h, name_map).
    The 24h fetch is isolated — if it fails, prices_24h is None but the rest still works.
    Uses cached name mapping to avoid re-fetching static data on every call."""
    prices_raw  = None
    volumes_raw = None
    prices_24h  = None
    name_map    = None

    # Core fetch — latest prices + 6h volume data (required for all categories)
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
            ...
            async with session.get(WIKI_6H_URL, timeout=aiohttp.ClientTimeout(total=10)) as r:
                ...
                # Build volumes_raw: total trades in last 6h = highPriceVolume + lowPriceVolume
                volumes_raw = {
    ...
    # 24h fetch — isolated so a failure only affects Trending categories
    try:
        async with aiohttp.ClientSession(headers=WIKI_HEADERS) as session:
        ...
        prices_24h = None  # Best Margins and High Volume still work fine
    ...
    return prices_raw, volumes_raw, prices_24h, name_map


async def fetch_item_price(item_name: str):
    """Look up a specific item by name. Returns (item_id, name, high, low, margin) or None."""
    prices_raw, volumes_raw, prices_24h, name_map = await fetch_ge_prices()
    if not prices_raw:
        return None
    query = item_name.lower().strip()
    exact_match_name = None
    # First pass: look for an exact name match
    for item_id, name in name_map.items():
        if name.lower() == query:
            exact_match_name = name
            data = prices_raw.get(item_id, {})
            high = data.get("high")
            low  = data.get("low")
            if high and low:
                margin = calc_margin(high, low)
                vol = volumes_raw.get(item_id, 0) if volumes_raw else 0
                return {"id": item_id, "name": name, "high": high, "low": low, "margin": margin, "volume": vol}
            else:
                # Item exists but has no current price data — return a clear no-price result
                return {"id": item_id, "name": name, "high": None, "low": None, "margin": None, "volume": 0}
    # Only fuzzy search if there was no exact match at all
...
async def build_flip_panel_embeds():
    ...
    FOOTER     = "📡 prices.runescape.wiki  •  🔄 Updates every 15 min"
    now        = datetime.now()
    ...
    if api_ok:
        candidates = []
        for item_id, data in prices_raw.items():
            high = data.get("high")
            low  = data.get("low")
            ...
            candidates.append((name, high, low, margin, vol))
        candidates.sort(key=lambda x: x[3], reverse=True)
        lines = []
        for i, (name, high, low, margin, vol) in enumerate(candidates[:TOP_MARGINS_COUNT], 1):
            roi = (margin / low * 100) if low else 0
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 Limit: {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
                f"{medal} **{name}**\n"
                f"🟢 Buy: **{fmt_exact(low)}**  🔴 Sell: **{fmt_exact(high)}**\n"
                f"📊 Margin: **{format_gp(margin)}**  ·  📈 ROI: {roi:.1f}%  ·  🔄 {vol:,}/6h{limit_str}"
            )
    ...
    if api_ok:
        ...
        for item_id, data in prices_raw.items():
            ...
            if margin < HV_MIN_MARGIN or vol < HV_MIN_VOLUME:
                continue
            name = name_map.get(item_id, f"Item #{item_id}")
            hv_candidates.append((name, high, low, margin, vol))
        hv_candidates.sort(key=lambda x: x[4], reverse=True)
        lines = []
        for i, (name, high, low, margin, vol) in enumerate(hv_candidates[:TOP_MARGINS_COUNT], 1):
            roi = (margin / low * 100) if low else 0
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 Limit: {buy_limit:,}/4h" if buy_limit else ""
    ...
    if api_ok and prices_24h:
        ...
        for item_id, data in prices_raw.items():
            ...
            up_candidates.append((name, current_high, avg_high, pct_change, gp_change, vol))
        up_candidates.sort(key=lambda x: x[3], reverse=True)
        lines = []
        for i, (name, current, avg, pct, gp_chg, vol) in enumerate(up_candidates[:TOP_MARGINS_COUNT], 1):
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 Limit: {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
                f"{medal} **{name}**\n"
                f"💰 Now: **{fmt_exact(current)}**  ·  📉 24h avg: {fmt_exact(avg)}\n"
    ...
    if api_ok and prices_24h:
        ...
        for item_id, data in prices_raw.items():
            ...
            name = name_map.get(item_id, f"Item #{item_id}")
            gp_change = current_high - avg_high
            down_candidates.append((name, current_high, avg_high, pct_change, gp_change, vol))
        down_candidates.sort(key=lambda x: x[3])
        lines = []
        for i, (name, current, avg, pct, gp_chg, vol) in enumerate(down_candidates[:TOP_MARGINS_COUNT], 1):
            medal = ["🥇","🥈","🥉"][i-1] if i <= 3 else f"`{i:>2}.`"
            buy_limit = ge_item_limit_map.get(item_id)
            limit_str = f"  ·  🛒 Limit: {buy_limit:,}/4h" if buy_limit else ""
            lines.append(
...
async def ge_price(ctx, *, item_name: str = None):
    ...
    embed.add_field(name="🟢 Instant Buy",        value=f"**{low:,} gp**",                           inline=True)
    embed.add_field(name="🔴 Instant Sell",       value=f"**{high:,} gp**",                          inline=True)
    ...
    embed.set_footer(text="📡 prices.runescape.wiki • Updates every ~60s • Always verify before committing")
    embed.timestamp = datetime.now()
    await ctx.send(embed=embed)
