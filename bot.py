        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                if resp.status == 200:
                    ...
                    return [m["player"]["displayName"].lower().replace(" ", "_") for m in data]
    except Exception as e:
...
async def wom_pending_check():
    ...
    for user_id, notified_at in list(wom_pending.items()):
        ...
        if hours_elapsed >= WOM_NOTIFY_HOURS:
            if log_channel:
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"{mention} verified as **{rsn}** but has not been added to the WOM group "
                        f"after **{WOM_NOTIFY_HOURS} hours**.\n\n"
                        f"🔗 https://wiseoldman.net/groups/{WOM_GROUP_ID}\n\n"
                        f"Please review and either:\n"
                        f"• Add them to WOM and use `!womclear @member` to dismiss this alert\n"
                        f"• Kick them from the server if they shouldn't be here"
                    ),
                    color=discord.Color.red()
                )
                embed.set_footer(text=f"Notified: {notified_dt.strftime('%Y-%m-%d %H:%M')} UTC")
                await log_channel.send(embed=embed)
            to_clear.append(user_id)
