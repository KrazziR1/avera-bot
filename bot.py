...
async def fetch_wom_members():
    ...
    try:
        url = f"https://api.wiseoldman.net/v2/groups/{WOM_GROUP_ID}/members?limit=500"
        headers = {"User-Agent": "Avera-Clan-Bot/1.0"}
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=15)) as resp:
                if resp.status == 200:
                    ...
                    print(f"⚠️ WOM API returned status {resp.status}")
                    return None
...
async def verify_rsn(ctx, *, rsn: str = None):
    ...
    if user_id not in member_ranks and is_first_verify:
        wom_members = await fetch_wom_members()
        if wom_members is not None and not rsn_in_wom(rsn, wom_members):
...
async def wom_check(ctx):
    ...
    msg = await ctx.send("🔍 Fetching Wise Old Man group members...")
    wom_members = await fetch_wom_members()

    if wom_members is None:
        await msg.edit(content="❌ Failed to fetch WOM group. Try again in a moment.")
        return
    ...
    for user_id, rsn in verified_rsns.items():
        if user_id in member_ranks:
            continue  # Skip staff
        if not rsn_in_wom(rsn, wom_members):
            member = ctx.guild.get_member(int(user_id))
            name = member.display_name if member else f"Unknown ({user_id})"
    ...
    embed = discord.Embed(
        ...
        description=f"The following **{len(missing)}** verified members are not in the WOM group:\n🔗 https://wiseoldman.net/groups/{WOM_GROUP_ID}",
        color=discord.Color.orange()
    )
    lines = "\n".join(f"• **{name}** — `{rsn}`" for name, rsn in missing)
    embed.add_field(name="Missing Members", value=lines[:1024], inline=False)
    embed.set_footer(text="Use !womcheck again after adding them to verify")
    await msg.edit(content=None, embed=embed)
...
async def wom_pending_check():
    ...
    for user_id, notified_at in list(wom_pending.items()):
        ...
        # If they're now in WOM, clear them
        if rsn_in_wom(rsn, wom_members):
            to_clear.append(user_id)
            continue
