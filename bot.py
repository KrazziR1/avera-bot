...
async def fetch_temple_stats(rsn):
    ...
    try:
        ...
        collection_log = info.get("Collection_log_count", None)
        if collection_log is not None:
            collection_log = int(collection_log)

        # Achievement diaries — TempleOSRS returns total diary tasks, we approximate tiers
        diaries = {
            "easy":   int(info.get("Diary_easy_complete", 0) or 0),
            "medium": int(info.get("Diary_medium_complete", 0) or 0),
            "hard":   int(info.get("Diary_hard_complete", 0) or 0),
            "elite":  int(info.get("Diary_elite_complete", 0) or 0),
        }
