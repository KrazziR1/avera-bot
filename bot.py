...
async def faq_panel(ctx):
    ...
    embed1.add_field(
        ...
        value=(
            ...
            f"`!referral @member` — Credit the person who referred you to Avera *(one time only, grants them +10 pts)*\n"
    ...
    embed1.add_field(
        name="🎖️ Ranks",
        value=(
            "Avera uses a **Glory** system — a permanent score earned through your time in the clan, "
            "level & XP milestones, bounties, referrals, and special bonuses awarded by staff.\n\n"
            "Ranks range from **Protector** up to **Templar**, with **Moderator** being staff-assigned only.\n\n"
            "Use `!glory` to see your score, `!rankprogress` for tips on ranking up, "
            "or check the rank breakdown further down in this channel!"
        ),
        inline=False
    )
    ...
    embed1.add_field(
        ...
        value=(
            ...
            f"📅 Use `!attend` during an active event — **+25 pts**\n"
            f"🔥 Attend 3 events in a row — **+5 bonus pts** streak reward\n"
            f"🌟 Host a clan event — **variable pts** *(set by admin per event)*\n"
            f"🥇 Finish 1st in a competition — **+50 pts**\n"
            f"🥈 Finish 2nd in a competition — **+25 pts**\n"
            f"🥉 Finish 3rd in a competition — **+10 pts**\n"
            f"🌱 Refer a new member who joins and verifies — **+10 pts**"
        ),
        inline=False
    )
    ...
    embed1.add_field(
        ...
        value=(
            ...
            f"`!attend` — Check in to an active event and earn **+25 points**\n"
            f"`!streak` — Check your consecutive event attendance streak\n"
            f"🔥 Attend **3 events in a row** to earn a streak bonus of **+5 points**"
        ),
        inline=False
    )
    ...
    # ── Embed 3: Rank System ───────────────────────────────────
    embed3 = discord.Embed(
        title="⚔️ The Avera Rank System",
        description=(
            "Ranks in Avera are earned through **Glory** — a permanent score that reflects everything "
            "you've contributed to the clan. Glory never decreases, so every bit of effort you put in "
            "counts forever, even if you spend your clan points in the shop.\n\n"
            "The more active, engaged and progressed you are as a player, the higher your Glory — "
            "and the higher your rank."
        ),
        color=discord.Color.gold()
    )
    ...
    for rank_name, threshold in reversed(GLORY_THRESHOLDS):
        ...
        rank_lines += f"{emoji} **{rank_name}**\n"

    embed3.add_field(
        name="🎖️ Ranks — Lowest to Highest",
        value=rank_lines.strip(),
        inline=False
    )
    ...
    embed3.add_field(
        name="✨ What is Glory?",
        value=(
            "Glory is your personal clan score — earned through a combination of:\n\n"
            "⏱️ **Time in the clan** — loyalty is rewarded\n"
            "📊 **Total Level milestones** — hit 500, 750, 1000, 1250, 1500, 1750, 2000, 2200, 2376\n"
            "🧮 **Total XP milestones** — hit 50M, 100M, 200M, 500M, 1B, 2B XP\n"
            "🎯 **Bounties claimed** — one of the biggest drivers, so get grinding!\n"
            "👥 **Referrals** — bringing new members into the clan\n\n"
            "You don't need to max everything to rank up — consistent activity and clan "
    ...
    embed3.add_field(
        name="📈 How do I check my rank progress?",
        value=(
            f"`!glory [@member]` — See your full Glory score and a breakdown of where it came from\n"
            f"`!rankprogress [@member]` — See exactly how much Glory you need for your next rank "
            f"and specific tips on how to earn it\n"
            f"`!profile [@member]` — View your full clan profile including your current rank and Glory progress bar\n"
            f"`!rankcheck` — Refresh your rank role based on your latest stats"
        ),
        inline=False
    )
    ...
    embed3.add_field(
        name="🛡️ Moderator",
        value=(
            "The **Moderator** rank sits above all earned ranks and is assigned directly by the clan owner "
            "to trusted members who help manage and support the community."
        ),
        inline=False
    )

    embed3.set_footer(text="Avera Clan • Glory is permanent — every contribution counts")
