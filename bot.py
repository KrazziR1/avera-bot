...
async def fetch_gear_image(wiki_name: str, style: str) -> tuple[bytes | None, str | None]:
    """
    Fetch the equipment section HTML from the OSRS Wiki, extract item images,
    and compile them into a gear grid PNG using Pillow.
    Returns (image_bytes, error_message).
    """
    import re, io
    from PIL import Image, ImageDraw, ImageFont

    # ── 1. Get the page sections to find the right one ───────────────────────
    try:
        session = http_session  # persistent session
        # Get section list
        params = {
            "action": "parse", "page": f"{wiki_name}/Strategies",
            "prop": "sections", "format": "json", "formatversion": "2"
        }
        async with session.get(WIKI_API_URL, params=params, headers=WIKI_HEADERS,
                               timeout=aiohttp.ClientTimeout(total=15)) as resp:
            if resp.status != 200:
                return None, "Wiki API unavailable"
            data = await resp.json()

        sections = data.get("parse", {}).get("sections", [])
        if not sections:
            # Try base page
            params["page"] = wiki_name
            async with session.get(WIKI_API_URL, params=params, headers=WIKI_HEADERS,
                                   timeout=aiohttp.ClientTimeout(total=15)) as resp:
                data = await resp.json()
            sections = data.get("parse", {}).get("sections", [])

        # Find the section index matching this style
        target_section = None
        for s in sections:
            if style.lower() in s.get("line", "").lower() or style.lower() in s.get("anchor", "").lower():
                target_section = s["index"]
                break

        # ── 2. Fetch rendered HTML for that section ───────────────────────
        page_title = f"{wiki_name}/Strategies"
        html_params = {
            "action": "parse", "page": page_title,
            "prop": "text", "format": "json", "formatversion": "2",
            "disableeditsection": "1",
        }
        # Don't filter by section — tab content needs full page fetch

        async with session.get(WIKI_API_URL, params=html_params, headers=WIKI_HEADERS,
                               timeout=aiohttp.ClientTimeout(total=20)) as resp:
            if resp.status != 200:
                return None, "Could not fetch wiki section"
            html_data = await resp.json()

        html = html_data.get("parse", {}).get("text", "")
        if not html:
            return None, "No content found for this style"

        # ── 3. Extract item image URLs from the HTML ──────────────────────
        # Wiki uses relative /images/thumb/... paths in rendered HTML
        img_pattern = re.compile(r'src="([^"]*?/images/(?:thumb/)?[^"]+\.png[^"]*)"', re.IGNORECASE)
        raw_urls = img_pattern.findall(html)
        datasrc_pattern = re.compile(r'data-src="([^"]*?/images/(?:thumb/)?[^"]+\.png[^"]*)"', re.IGNORECASE)
        raw_urls += datasrc_pattern.findall(html)

        seen = set()
        item_urls = []
        WIKI_BASE = "https://oldschool.runescape.wiki"
        SKIP_KEYWORDS = ["Coins", "coins", "interface", "Icon_", "icon_", "logo", "Logo",
                         "Chat", "Hitsplat", "Prayer_icon", "Attack_icon", "Strength_icon",
                         "Defence_icon", "Ranged_icon", "Magic_icon", "Hitpoints_icon",
                         "Slayer_icon", "cursor", "Cursor", "Map_", "map_"]

        for url in raw_urls:
            if url.startswith("//"):
                full_url = "https:" + url
            elif url.startswith("/"):
                full_url = WIKI_BASE + url
            elif url.startswith("http"):
                full_url = url
            else:
                continue
            # Strip thumbnail sizing suffix
            clean = re.sub(r'(/images)/thumb(/.+\.png)/\d+px-[^/?]+', r'\1\2', full_url)
            clean = clean.split("?")[0]
            if clean in seen:
                continue
            if any(kw in clean for kw in SKIP_KEYWORDS):
                continue
            # Skip tiny UI icons (pixel size in filename < 30px)
            m = re.search(r'/(\d+)px-', full_url)
            if m and int(m.group(1)) < 30:
                continue
            seen.add(clean)
            item_urls.append(clean)

        if not item_urls:
            return None, "Could not find gear images for this style"

        item_urls = item_urls[:20]

        # ── 4. Download item images ───────────────────────────────────────
        item_images = []
        for url in item_urls:
            try:
                async with session.get(url, headers=WIKI_HEADERS,
        ...
        if not item_images:
            return None, "Could not download gear images"

    except Exception as e:
        return None, f"Fetch error: {e}"

    # ── 5. Build the gear grid image ──────────────────────────────────────────
    try:
        ITEM_SIZE  = 48   # Each item icon resized to 48x48
        COLS       = 5
        PADDING    = 8
        BG_COLOUR  = (30, 31, 34, 255)   # Discord dark background
        BORDER_COL = (70, 73, 80, 255)

        rows = (len(item_images) + COLS - 1) // COLS
        W = COLS * (ITEM_SIZE + PADDING) + PADDING
        H = rows * (ITEM_SIZE + PADDING) + PADDING + 30  # +30 for title bar
        ...
        draw.text((PADDING, 6), f"{wiki_name} — {style}", fill=(255, 255, 255, 255), font=font)
        ...
        for i, img in enumerate(item_images):
            ...
            y = 30 + PADDING + row * (ITEM_SIZE + PADDING)

            # Background cell
            draw.rectangle([x-2, y-2, x+ITEM_SIZE+2, y+ITEM_SIZE+2], fill=BORDER_COL)

            # Resize and paste item
            resized = img.resize((ITEM_SIZE, ITEM_SIZE), Image.LANCZOS)
        ...
        return None, f"Image build error: {e}"
...
class GearStyleView(discord.ui.View):
    ...
    def _make_callback(self, style: str):
        async def callback(interaction: discord.Interaction):
            ...
            img_bytes, err = await fetch_gear_image(self.wiki_name, style)
...
async def gear_command(ctx, *, boss: str = None):
    ...
    if not wiki_name:
        ...
        for alias, name in BOSS_ALIASES.items():
            if boss_lower in alias or alias in boss_lower:
                ...
                break
    if not wiki_name:
        # Use the input directly, title-cased
        wiki_name = boss.strip().title()

    if not http_session or http_session.closed:
        await ctx.send("⚠️ Bot session not ready yet — try again in a moment.")
        return

    thinking_msg = await ctx.send(f"🔍 Looking up **{wiki_name}** on the OSRS Wiki...")

    # Fetch wikitext
    try:
        wikitext = await fetch_wiki_wikitext(wiki_name)
    except Exception as e:
        await thinking_msg.edit(content=f"❌ Error fetching wiki: `{e}`")
        return
    if not wikitext:
        await thinking_msg.edit(content=f"❌ Couldn't find a strategies page for **{wiki_name}** on the OSRS Wiki.\nCheck the spelling or try the full name.")
        return

    # Parse available combat styles
    styles = parse_combat_styles_from_wikitext(wikitext)
    if not styles:
        # Fallback — offer all three and let Claude figure it out
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"inventory, prayers, and settings.\n\n"
    ...
    view = GearStyleView(boss_lower, wiki_name, styles, wikitext)
